...
function print_work_order_listing()
{
	...
	$open = $open_only == 1 ? trans('Yes') : trans('No');
	$show = $show_gl == 1 ? trans('Yes') : trans('No');
	
	$cols = array(0, 100, 120, 165, 210, 275, 315, 375, 385, 440, 495, 555);

	$headers = array(trans('Type'), '#', ('Reference'), trans('Location'), trans('Item'), trans('Required'), trans('Manufactured'), ' ', trans('Date'), trans('Required By'), trans('Closed'));

	if ($show_gl)
	{
		$cols2 = $cols;
		$headers2 = array(trans("Transaction"), ' ', trans("Date"), trans("Account Code"),' ' . trans("Account Name"), trans("Debit"), trans("Credit"), ' ', trans("Memo"));
	}	
	else
	{
		$cols2 = null;
    ...
    $params =   array( 	0 => $comments,
    				    1 => array('text' => trans('Items'), 'from' => $items, 'to' => ''),
    				    2 => array('text' => trans('Location'), 'from' => $loc, 'to' => ''),
    				    3 => array('text' => trans('Open Only'), 'from' => $open, 'to' => ''),
    				    4 => array('text' => trans('Show GL Rows'), 'from' => $show, 'to' => ''),
    				    );

    $rep = new FrontReport(trans('Work Order Listing'), "WorkOrderListing", user_pagesize(), 9, $orientation);
   	if ($orientation == 'L')
	...
	while ($trans=db_fetch($res))
	{
		...
		$rep->TextCol(10, 11, $trans['closed'] ? ' ' : trans('No'), -1);
		if ($show_gl)
		{
			$rep->NewLine();
			$productions = get_gl_wo_productions($trans['id'], true);
			print_gl_rows($rep, $productions, trans("Finished Product Requirements"));

			$issues = get_gl_wo_issue_trans($trans['id'], -1, true);
			print_gl_rows($rep, $issues, trans("Additional Material Issues"));

		    $costs = get_gl_wo_cost_trans($trans['id'], -1, true);
			print_gl_rows($rep, $costs, trans("Additional Costs"));

			$wo = get_gl_trans(ST_WORKORDER, $trans['id']);
			print_gl_rows($rep, $wo, trans("Finished Product Receival"));
			$rep->Line($rep->row - 2);
			$rep->NewLine();
		}
		$rep->NewLine();
