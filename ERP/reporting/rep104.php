...
function fetch_items($category=0)
{
		...
		$sql .= " AND item.mb_flag<> '".STOCK_TYPE_FIXED_ASSET."' ORDER BY item.category_id,
				item.stock_id";
...
function print_price_listing()
{
	...
	$headers = array(trans('Category/Items'), trans('Description'),	trans('UOM'), trans('Price'),	trans('GP %'));

	$aligns = array('left',	'left',	'left', 'right', 'right');

    $params =   array( 	0 => $comments,
    				    1 => array('text' => trans('Currency'), 'from' => $curr_sel, 'to' => ''),
    				    2 => array('text' => trans('Category'), 'from' => $cat, 'to' => ''),
    				    3 => array('text' => trans('Sales Type'), 'from' => $stype, 'to' => ''),
    				    4 => array(  'text' => trans('Show GP %'),'from' => $GP,'to' => ''));
    ...
    $rep = new FrontReport(trans('Price Listing'), "PriceListing", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
	...
	while ($myrow=db_fetch($result))
	{
		if ($catgor != $myrow['cat_name'])
		{
			if ($catgor == '')
			{
				$rep->NewLine(2);
				$rep->fontSize += 2;
				$rep->TextCol(0, 3, trans("Sales Kits"));
				$rep->fontSize -= 2;
			}
			$rep->Line($rep->row  - $rep->lineHeight);
			$rep->NewLine(2);
