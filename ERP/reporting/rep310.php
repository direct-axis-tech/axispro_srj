...
function print_inventory_purchase()
{
	...
	$headers = array(trans('Item'), trans('Description'), trans('Date'), trans('#'), trans('Supplier'), trans('Qty'), trans('Unit Price'), trans('Location'));
	if ($fromsupp != '')
		$headers[4] = '';
    ...
    $params =   array( 	0 => $comments,
    				    ...
    				    4 => array('text' => trans('Supplier'), 'from' => $froms, 'to' => ''),
    				    5 => array('text' => trans('Item'), 'from' => $itm, 'to' => ''),
                    	6 => array('text' => trans('Note'), 'from' => trans('The lines separate the transactions.'),
                    		'to' => ''));

    $rep = new FrontReport(trans('Inventory Purchasing - Transaction Based'), "InventoryPurchasingTransactionsReport", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
