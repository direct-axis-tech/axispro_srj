...
function print_list_of_journal_entries()
{
    ...
    $headers = array(trans('Type/Account'), trans('Reference').'/'.trans('Account Name'), trans('Date/Dim.'),
    	trans('Person/Item/Memo'), trans('Debit'), trans('Credit'));

    $aligns = array('left', 'left', 'left', 'left', 'right', 'right');

    $params =   array( 	0 => $comments,
    				    1 => array('text' => trans('Period'), 'from' => $from,'to' => $to),
                    	2 => array('text' => trans('Type'), 'from' =>
						$systype == -1 ? trans('All') : $systypes_array[$systype],
                            'to' => ''));

    $rep = new FrontReport(trans('List of Journal Entries'), "JournalEntries", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
    ...
    while ($myrow=db_fetch($trans))
    {
        ...
        $rep->TextCol(3, 6, $myrow['person_name']);
