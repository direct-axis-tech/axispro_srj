...
function print_invoices()
{
	...
	$cols = array(4, 80, 200, 240,370, 440, 480);

	// $headers in doctext.inc
	$aligns = array('left',	'left',	'right', 'center', 'center', 'center', 'center','right','right');
	...
	if ($email == 0)
		$rep = new FrontReport(trans('INVOICE'), "InvoiceBulk", user_pagesize(), 9, $orientation);
	if ($orientation == 'L')
...
//			$rep->formData['customer_mobile'] = $myrow['customer_mobile'];
...
//                display_error(print_r($this->line_items ,true));
...
//				$SubTotal += $Net;
	    		...
	    		//Modified for AMER
	    		$DiscountedAmount = ($DisplayPrice*$DisplayDiscount)/100;

                $DiscountedAmountTotal += $DiscountedAmount;

                $DisplayNet = number_format2(floatval($DisplayNet)+$DiscountedAmount,$dec);

                $SubTotal += floatval($DisplayNet);
...
//					$rep->TextCol($c++, $c,	$myrow2['govt_fee'], -2);
					$rep->TextCol($c++, $c,	number_format2($myrow2['govt_fee']+$myrow2['bank_service_charge']+$myrow2['bank_service_charge_vat'],$dec), -2);
					$rep->TextCol($c++, $c,	$DisplayPrice, -2);
//					$rep->TextCol($c++, $c,	$DiscountedAmount, -2);
			...
			if (isset($prepayments))
			{
				...
				$rep->TextCol(0, 3,trans("Prepayments invoiced to this order up to day:"));
				$rep->TextCol(0, 3,	str_pad('', 150, '_'));
				$rep->cols[2] -= 20;
				$rep->aligns[2] = 'right';
				$rep->NewLine(); $c = 0; $tot_pym=0;
				$rep->TextCol(0, 3,	str_pad('', 150, '_'));
				$rep->TextCol($c++, $c, trans("Date"));
				$rep->TextCol($c++, $c,	trans("Invoice reference"));
				$rep->TextCol($c++, $c,	trans("Amount"));
				...
				$rep->TextCol(0, 3,	str_pad('', 150, '_'));
				$rep->NewLine();
				$rep->TextCol(1, 2,	trans("Total payments:"));
			...
			$rep->TextCol(3, 8, trans("Sub-total"), -2);
			$rep->TextCol(5, 8,	$DisplaySubTot, -2);
			$rep->NewLine();
			if ($myrow['ov_freight'] != 0.0)
			{
   				$DisplayFreight = number_format2($sign*$myrow["ov_freight"],$dec);
				$rep->TextCol(3, 8, trans("Shipping"), -2);
				$rep->TextCol(5, 8,	$DisplayFreight, -2);
				$rep->NewLine();
			}
    		...
    		while ($tax_item = db_fetch($tax_items))
    		{
    			...
    			if ($myrow['tax_included'])
    			{
    				if ($SysPrefs->alternative_tax_include_on_docs() == 1)
    				{
    					if ($first)
    					{
							$rep->TextCol(3, 8, trans("Total Tax Excluded"), -2);
							$rep->TextCol(5, 8,	number_format2($sign*$tax_item['net_amount'], $dec), -2);
							$rep->NewLine();
    					}
						$rep->TextCol(3, 8, $tax_type_name, -2);
						$rep->TextCol(5, 8,	$DisplayTax, -2);
						$first = false;
    				}
    				else
						$rep->TextCol(3, 8, trans("Included") . " " . $tax_type_name . trans("Amount") . ": " . $DisplayTax, -2);
				}
    			else
    			{
					$rep->TextCol(3, 8, $tax_type_name, -2);
			...
			$DisplayTotal = number_format2($sign*($myrow["ov_freight"] + $myrow["ov_gst"] +
				$myrow["ov_amount"]+$myrow["ov_freight_tax"]+$DiscountedAmountTotal),$dec);
			$rep->Font('bold');
			if (!$myrow['prepaid']) $rep->Font('bold');
				$rep->TextCol(3, 8, $rep->formData['prepaid'] ? trans("TOTAL ORDER VAT INCL.") : trans("TOTAL INVOICE"), - 2);
			$rep->TextCol(5, 8, $DisplayTotal, -2);
			if ($rep->formData['prepaid'])
			{
				$rep->NewLine();
				$rep->Font('bold');
				$rep->TextCol(3, 8, $rep->formData['prepaid']=='final' ? trans("THIS INVOICE") : trans("TOTAL INVOICE"), - 2);
				$rep->TextCol(5, 8, number_format2($myrow['prep_amount'], $dec), -2);
			}
