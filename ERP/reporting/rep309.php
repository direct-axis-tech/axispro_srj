...
function getTransactions($category, $from, $to)
{
	...
	$sql = "SELECT item.category_id,
		...
		AND line.quantity<>0
		AND item.mb_flag <>'".STOCK_TYPE_FIXED_ASSET."'
...
function print_inventory_sales()
{
	...
	$headers = array(trans('Item/Category'), trans('Description'), trans('Qty'), trans('Unit Price'), trans('Sales'), '', trans('Remark'));
    ...
    $rep = new FrontReport(trans('Item Sales Summary Report'), "ItemSalesSummaryReport", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
