...
function print_balance_sheet()
{
	...
	$headers = array(trans('Account'), trans('Account Name'), trans('Open Balance'), trans('Period'),
		trans('Close Balance'));
	...
	$rep = new FrontReport(trans('Balance Sheet'), "BalanceSheet", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
	...
	$rep->Font('bold');	
	$rep->TextCol(0, 2,	trans('Total') . " " . trans('Liabilities') . trans(' and ') . trans('Equities'));
