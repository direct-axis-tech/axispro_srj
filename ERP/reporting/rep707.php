...
function print_profit_and_loss_statement()
{
	...
	$cols = array(0, 100, 300, 500, 700,	800);
	//------------0--1---2----3----4----5--

	$headers = array(trans('Account'), trans('Account Name'), trans('Amount'));
	...
	elseif ($compare == 1)
	{
		...
		$headers[3] = trans('Period Y-1');
	}

	$rep = new FrontReport(trans('Profit and Loss Statement'), "ProfitAndLoss", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
...
//	$rep->AmountCol(4, 5, Achieve($salesper, $salesacc), $pdec);
	...
	if ($graphics)
	{
		$labels[] = trans('Calculated Return');
		$serie1[] = abs($salesper);
		$serie2[] = abs($salesacc);
		$pg->setStream('png');
		$pg->setLabels($labels);
		...
		$pg->setXTitle(trans("Group"));
		$pg->setYTitle(trans("Amount"));
