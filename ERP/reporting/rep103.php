...
function print_customer_details_listing()
{
	...
	if ($area == 0)
		$sarea = trans('All Areas');
	else
		$sarea = get_area_name($area);
	if ($folk == 0)
		$salesfolk = trans('All Sales Folk');
	else
		$salesfolk = get_salesman_name($folk);
	if ($more != '')
		$morestr = trans('Greater than ') . number_format2($more, $dec);
	else
		$morestr = '';
	if ($less != '')
		$lessstr = trans('Less than ') . number_format2($less, $dec);
	else
		$lessstr = '';
	...
	$headers = array(trans('Customer Postal Address'), trans('Price/Turnover'),	trans('Branch Contact Information'),
		trans('Branch Delivery Address'));

	$aligns = array('left',	'left',	'left',	'left');

    $params =   array( 	0 => $comments,
    				    1 => array('text' => trans('Activity Since'), 	'from' => $from, 		'to' => ''),
    				    2 => array('text' => trans('Sales Areas'), 		'from' => $sarea, 		'to' => ''),
    				    3 => array('text' => trans('Sales Folk'), 		'from' => $salesfolk, 	'to' => ''),
    				    4 => array('text' => trans('Activity'), 		'from' => $morestr, 	'to' => $lessstr . " " . get_company_pref("curr_default")));

    $rep = new FrontReport(trans('Customer Details Listing'), "CustomerDetailsListing", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
	...
	while ($myrow=db_fetch($result))
	{
		...
		if ($printcustomer)
		{
			...
			if ($carea != $myrow['description'])
			{
				...
				$rep->NewLine(2, 7);
				$rep->Font('bold');
				$rep->TextCol(0, 3,	trans('Customers in') . " " . $myrow['description']);
			...
			$rep->TextCol(1, 2,	trans('Price List') . ": " . $myrow['sales_type']);
			...
			$rep->TextCol(1, 2,	trans('Currency') . ": " . $myrow['curr_code']);
			if (isset($contacts[0]))
			...
			if ($myrow['dimension_id'] != 0)
			{
				...
				$rep->TextCol(1, 2,	trans('Dimension') . ": " . $dim['name']);
			}		
			if (isset($contacts[0]))
				$rep->TextCol(2, 3, trans('Ph') . ": " . $contacts[0]['phone']);
			if (isset($adr2[1]))
			...
			if ($myrow['dimension2_id'] != 0)
			{
				...
				$rep->TextCol(1, 2,	trans('Dimension') . " 2: " . $dim['name']);
			}	
			if ($myrow['notes'] != '')
			{
				$oldrow = $rep->row;
				$rep->NewLine();
				$rep->TextColLines(1, 2, trans("General Notes:")." ".$myrow['notes'], -2);
				$newrow = $rep->row;
				$rep->row = $oldrow;
			}	
			if (isset($contacts[0]))
				$rep->TextCol(2, 3, trans('Fax') . ": " . $contacts[0]['fax']);
			if (isset($adr2[2]))
				$rep->TextCol(3, 4, $adr2[2]);
			if ($more != 0.0 || $less != 0.0)
				$rep->TextCol(1, 2,	trans('Turnover') . ": " . number_format2($turnover, $dec));
			for ($i = 3; $i < $count1; $i++)
			{
				$rep->NewLine();
				if (isset($adr[$i]))
				...
				if ($i == 3 && isset($contacts[0]) && isset($contacts[0]['email']))	
					$rep->TextCol(2, 3, trans('Email') . ": " . $contacts[0]['email']);
				if (isset($adr2[$i]))
