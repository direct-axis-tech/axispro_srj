...
function print_statements()
{
	...
	while ($myrow=db_fetch($result))
	{
		...
		$rep->fontSize += 2;
		$rep->TextCol(0, 8, trans("Outstanding Transactions"));
		$rep->fontSize -= 2;
		$rep->NewLine(2);
		...
		while ($myrow2=db_fetch($TransResult))
		{
            ...
            $total_outstanding += $myrow2["TotalAmount"] - $myrow2["Allocated"];
		...
		$pastdue2 = trans("Over") . " " . $PastDueDays2 . " " . trans("Days");
		$CustomerRecord = get_customer_details($myrow['debtor_no'], null, $show_also_allocated);
		$str = array(trans("Current"), $nowdue, $pastdue1, $pastdue2, trans("Total Balance"));
...
//		if ($email == 1)
//			$rep->End($email, trans("Statement") . " " . trans("as of") . " " . sql2date($date));
        ...
        $rep->TextCol(3, 4,	"");
        $rep->TextCol(4, 5,	"");
        $rep->TextCol(5, 6,	"Total Outstanding");
        $rep->TextCol(6, 7,	"");
        $rep->TextCol(7, 8,	$total_outstanding);
