...
$page_security = $_POST['PARAM_0'] == $_POST['PARAM_1'] ? 'SA_PRINTRCPTRFND' : 'SA_SALESBULKREP';
...
//----------------------------------------------------------------------------------------------------
function get_refund($type, $trans_no, $voided=false)
{
    $prefix = $voided ? '0_voided' : '0';
    $sql = (
        "SELECT trans.*,
        ...
        FROM
            {$prefix}_debtor_trans trans
    		LEFT JOIN {$prefix}_bank_trans bTrans ON bTrans.type = trans.type and bTrans.trans_no = trans.trans_no
        ...
        WHERE
            trans.type = ".db_escape($type)."
            AND trans.trans_no = ".db_escape($trans_no)
    );
	
   	$result = db_query($sql, "The refunds cannot be retrieved");
...
function print_refund()
{
	...
	for ($trans_no = $from; $trans_no <= $to; $trans_no++) {
		foreach ($types as $type) {
			if (
				!($myrow = get_refund($type, $trans_no, $voided)) ||
				($currency != ALL_TEXT && $myrow['curr_code'] != $currency)
			) {
				continue;
			}

			$allocations = get_allocatable_from_cust_transactions($myrow['debtor_no'], $myrow['trans_no'], $myrow['type'])->fetch_all(MYSQLI_ASSOC);
			$contents[] = get_contents($myrow, $allocations, $generator);
			$trans = $myrow;
		}
	}

	$file_name = 'refund-'.($fno[0] == $tno[0] ? ($trans['reference'] ?? '') : random_id()).'.pdf';
...
function get_contents($myrow, $allocations, $generator) {
	...
	<div class="text-center pt-3">
		<h2 class="font-weight-normal">
            <b><span class="pb-3">REFUND &nbsp;-&nbsp;<span lang="ar">استرداد</span></span></b>
		</h2>
	</div>
				...
				<td class="border w-30">
					<table class="w-100">
						<tbody>
							...
							<tr>
                                <td><b>From Account/<span lang="ar">من الحساب</span></b></td>
								<td><?= $myrow['bank_account_name'] ?></td>
							</tr>
						</tbody>
					</table>
				</td>
				<td class="border w-70 align-bottom">
					<table class="w-100">
						<tbody>
							<tr>
								<td><b>Created At/<span lang="ar">أنشئت في</span></b></td>
	...
	<table class="table-sm text-center table-bordered w-100 mt-4">
		<thead>
			<tr>
                ...
                <th style="width: 23%">Payment Ref<br><span lang="ar">مرجع الدفع</span></th>
                <th style="width: 24%">Payment Amount<br><span lang="ar">مبلغ الدفع</span></th>
		...
		<tfoot class="text-right">
			<tr>
                <td colspan="4">TOTAL REFUND AMOUNT</td>
			...
			<?php if ($myrow['commission']): ?>
			<tr>
				<td colspan="4">COMMISSION</td>
				<td><?= price_format($myrow['commission']) ?></td>
			</tr>
			<?php endif; ?>
            ...
            <tr>
                <td colspan="4"><b>CASH REFUNDED</b></td>
                <td><b><?= price_format($myrow['Total'] + $myrow['credit_card_charge'] + $myrow['round_of_amount'] - $myrow['ov_discount'] - $myrow['commission']) ?></b></td>
            </tr>

			<tr>
                <td colspan="5" class="text-center">
                    <span>This refund is generated electronically</span>
					<span> - </span>
