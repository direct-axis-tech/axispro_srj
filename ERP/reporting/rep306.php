...
function print_inventory_purchase()
{
	...
	$headers = array(trans('Category'), trans('Description'), trans('Date'), trans('#'), trans('Supplier'), trans('Qty'), trans('Unit Price'), trans('Total'));
	if ($fromsupp != '')
		$headers[4] = '';
    ...
    $rep = new FrontReport(trans('Inventory Purchasing Report'), "InventoryPurchasingReport", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
	...
	$rep->NewLine();
	$rep->NewLine(2, 1);
	$rep->TextCol(0, 7, trans('Grand Total'));
