...
function print_audit_trail()
{
    ...
    $headers = array(trans('Date'), trans('Time'), trans('User'), trans('Trans Date'),
    	trans('Type'), trans('#'), trans('Action'), trans('Amount'));
    ...
    $params =   array( 	0 => $comments,
                    	...
                    	2 => array('text' => trans('Type'), 'from' => ($systype != -1 ? $systypes_array[$systype] : trans('All')), 'to' => ''),
                    	3 => array('text' => trans('User'), 'from' => ($user != -1 ? $user_id : trans('All')), 'to' => ''));

    $rep = new FrontReport(trans('Audit Trail'), "AuditTrail", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
