...
function print_fixed_assets_valuation_report()
{
	...
	$headers = array(trans('Class'), '', trans('UOM'),  trans('Initial'), trans('Depreciations'), trans('Current'));

	$aligns = array('left',	'left',	'left', 'right', 'right', 'right', 'right');

    $params =   array( 	0 => $comments,
    					1 => array('text' => trans('End Date'), 'from' => $date, 		'to' => ''),
    				    2 => array('text' => trans('Class'), 'from' => $cln, 'to' => ''),
    				    3 => array('text' => trans('Location'), 'from' => $loc, 'to' => ''));

    $rep = new FrontReport(trans('Fixed Assets Valuation Report'), "FixedAssetsValReport", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
