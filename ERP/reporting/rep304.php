...
function getTransactions($category, $location, $fromcust, $from, $to, $show_service)
{
	...
	if (!$show_service)
		$sql .= " AND (item.mb_flag='".STOCK_TYPE_PURCHASED."' OR item.mb_flag='".STOCK_TYPE_MANUFACTURED."')";
	else
		$sql .= " AND item.mb_flag<>'".STOCK_TYPE_FIXED_ASSET."'";
	if ($category != 0)
...
function print_inventory_sales()
{
	...
	if ($show_service) $show_service_items = trans('Yes');
	else $show_service_items = trans('No');

	$cols = array(0, 75, 175, 250, 300, 375, 450,	515);

	$headers = array(trans('Category'), trans('Description'), trans('Customer'), trans('Qty'), trans('Sales'), trans('Cost'), trans('Contribution'));
	if ($fromcust != '')
		$headers[2] = '';
    ...
    $params =   array( 	0 => $comments,
    				    ...
    				    2 => array('text' => trans('Category'), 'from' => $cat, 'to' => ''),
    				    3 => array('text' => trans('Location'), 'from' => $loc, 'to' => ''),
    				    4 => array('text' => trans('Customer'), 'from' => $fromc, 'to' => ''),
    				    5 => array('text' => trans('Show Service Items'), 'from' => $show_service_items, 'to' => ''));

    $rep = new FrontReport(trans('Inventory Sales Report'), "InventorySalesReport", user_pagesize(), 9, $orientation);
   	if ($orientation == 'L')
	...
	while ($trans=db_fetch($res))
	{
		...
		{
			$rep->TextCol(1, 2, $trans['description'].($trans['inactive']==1 ? " (".trans("Inactive").")" : ""), -1);
			$rep->TextCol(2, 3, $trans['debtor_name']);
		}
		else
			$rep->TextCol(1, 3, $trans['description'].($trans['inactive']==1 ? " (".trans("Inactive").")" : ""), -1);
	...
	$rep->NewLine();
	$rep->NewLine(2, 1);
	$rep->TextCol(0, 4, trans('Grand Total'));
