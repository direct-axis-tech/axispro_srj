...
function fetch_items($category=0)
{
		$sql = "SELECT stock_id, stock.description AS name,
			...
			FROM ".TB_PREF."stock_master stock LEFT JOIN ".TB_PREF."stock_category cat ON stock.category_id=cat.category_id
				WHERE mb_flag <> '".STOCK_TYPE_SERVICE."' AND mb_flag <> '".STOCK_TYPE_FIXED_ASSET."'";
		if ($category != 0)
...
function inventory_movements()
{
	...
	$headers = array(trans('Category'), trans('Description'),	trans('UOM'), '', '', trans('OpeningStock'), '', '',trans('StockIn'), '', '', trans('Delivery'), '', '', trans('ClosingStock'));
	$headers2 = array("", "", "", trans("QTY"), trans("Rate"), trans("Value"), trans("QTY"), trans("Rate"), trans("Value"), trans("QTY"), trans("Rate"), trans("Value"), trans("QTY"), trans("Rate"), trans("Value"));
    ...
    $rep = new FrontReport(trans('Costed Inventory Movements'), "CostedInventoryMovements", user_pagesize(), 8, $orientation);
    if ($orientation == 'L')
	...
	$rep->Line($rep->row  - 4);
	$rep->NewLine(2);
	$rep->TextCol(0, 1,	trans("Total Movement"));
	...
	$rep->NewLine(1);
	$rep->TextCol(0, 1,	trans("Total Out"));
