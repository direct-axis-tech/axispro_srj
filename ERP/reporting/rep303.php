...
function getTransactions($category, $location, $item_like)
{
	$sql = "SELECT item.category_id,
		...
		AND (item.mb_flag='".STOCK_TYPE_PURCHASED."' OR item.mb_flag='".STOCK_TYPE_MANUFACTURED."')";
	if ($category != 0)
...
function print_stock_check()
{
	...
	if ($check)
	{
		...
		$headers = array(trans('Stock ID'), trans('Description'), trans('UOM'), trans('Quantity'), trans('Check'), trans('Demand'), $available, trans('On Order'));
		$aligns = array('left',	'left',	'left', 'right', 'right', 'right', 'right', 'right');
	}
	else
	{
		$cols = array(0, 75, 225, 250, 315, 380, 445,	515);
		$headers = array(trans('Stock ID'), trans('Description'), trans('UOM'), trans('Quantity'), trans('Demand'), $available, trans('On Order'));
		$aligns = array('left',	'left',	'left', 'right', 'right', 'right', 'right');
	}

    $params =   array(
		0 => $comments,
    	1 => array('text' => trans('Category'), 'from' => $cat, 'to' => ''),
    	2 => array('text' => trans('Location'), 'from' => $loc, 'to' => ''),
    	3 => array('text' => trans('Only Shortage'), 'from' => $short, 'to' => ''),
		4 => array('text' => trans('Suppress Zeros'), 'from' => $nozeros, 'to' => '')
	);
	...
	if ($barcodes)
	{
   	...
   	$rep = new FrontReport(trans('Stock Check Sheets'), "StockCheckSheet", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
