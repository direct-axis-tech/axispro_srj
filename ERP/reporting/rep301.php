...
function getTransactions($category, $location, $date)
{
	...
	$sql = "SELECT item.category_id,
		...
		AND item.mb_flag <> '".STOCK_TYPE_SERVICE."'
...
function print_inventory_valuation_report()
{
	...
	$headers = array(trans('Category'), '', trans('UOM'), trans('Quantity'), trans('Unit Cost'), trans('Value'));

	$aligns = array('left',	'left',	'left', 'right', 'right', 'right');

    $params =   array( 	0 => $comments,
    					1 => array('text' => trans('End Date'), 'from' => $date, 		'to' => ''),
    				    2 => array('text' => trans('Category'), 'from' => $cat, 'to' => ''),
    				    3 => array('text' => trans('Location'), 'from' => $loc, 'to' => ''));

    $rep = new FrontReport(trans('Inventory Valuation Report'), "InventoryValReport", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
