...
function print_bank_transactions()
{
	...
	$rep = new FrontReport(trans('Bank Statement'), "BankStatement", user_pagesize(), 9, $orientation);
	$dec = user_price_dec();
	...
	$headers = array(trans('Type'),	trans('#'),	trans('Reference'), trans('Date'), trans('Person/Item'),
		trans('Debit'),	trans('Credit'), trans('Balance'));
	...
	while ($account=db_fetch($result))
	{
		...
		$params =   array( 	0 => $comments,
			...
			2 => array('text' => trans('Bank Account'),'from' => $act,'to' => ''));
	...
	if ($prev_balance != 0.0 || $rows != 0)
		{
			$rep->Font('bold');
			$rep->TextCol(0, 3,	$act);
			$rep->TextCol(3, 5, trans('Opening Balance'));
			if ($prev_balance > 0.0)
			...
			$rep->TextCol(3, 5, trans("Total Debit / Credit"));
			$rep->AmountCol(5, 6, $total_debit, $dec);
			$rep->AmountCol(6, 7, $total_credit, $dec);
			$rep->NewLine(2);

			$rep->Font('bold');
			$rep->TextCol(3, 5,	trans("Ending Balance"));
			if ($total > 0.0)
			...
			$rep->TextCol(3, 5, trans("Net Change"));
			if ($total > 0.0)
