...
function GetSalesmanTrans($from, $to)
{
	...
	$sql = "SELECT DISTINCT trans.*,
			sum(trans_details.unit_price)*sum(quantity) AS InvoiceTotal,
		...
		FROM ".TB_PREF."debtor_trans trans,
			...
			".TB_PREF."debtor_trans_details trans_details 
		WHERE sorder.order_no=trans.order_
		    ...
		    AND trans_details.debtor_trans_type=10 AND  trans_details.debtor_trans_no=trans.trans_no AND  trans_details.debtor_trans_type=trans.type 
...
function print_salesman_list()
{
	...
	$headers = array(trans('Invoice'), trans('Customer'), trans('Branch'), trans('Customer Ref'),
		trans('Inv Date'),	trans('Total'),	trans('Provision'));

	$aligns = array('left',	'left',	'left', 'left', 'left', 'right',	'right');

	$headers2 = array(trans('Salesman'), " ",	trans('Phone'), trans('Email'),	trans('Provision'),
		trans('Break Pt.'), trans('Provision')." 2");

    $params =   array( 	0 => $comments,
	    				1 => array(  'text' => trans('Period'), 'from' => $from, 'to' => $to),
	    				2 => array(  'text' => trans('Summary Only'),'from' => $sum,'to' => ''));

	$aligns2 = $aligns;

	$rep = new FrontReport(trans('Salesman Listing'), "SalesmanListing", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
