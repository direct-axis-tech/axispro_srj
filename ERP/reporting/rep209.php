...
function print_po()
{
    ...
    $cols = array(4, 100, 300, 340, 385, 450, 515);
    ...
    if ($email == 0)
        $rep = new FrontReport(trans('PURCHASE ORDER'), "PurchaseOrderBulk", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
    ...
    for ($i = $from; $i <= $to; $i++) {
        ...
        if ($email == 1) {
            ...
            $rep->title = trans('PURCHASE ORDER');
        ...
        while ($myrow2 = db_fetch($result)) {
            ...
            $rowBefore = $rep->row;
            $rep->TextColLines(1, 2, $myrow2['description'], -3, 0, 0, 0, null, 0, 7);
            $rowAfter = $rep->row;
            $rep->row = $rowBefore;
        ...
        if ($myrow['comments'] != "") {
            $rep->NewLine();
            $rep->TextCol(1, 2, "  COMMENTS");
            $rep->NewLine();
            $rep->TextCol(1, 2, "  ...................");
            $rep->NewLine();
            $rep->NewLine();
            $rep->TextColLines(1, 2, $myrow['comments'], -3, 0, 0, 0, null, 0, 7);
        }

        if ($myrow['terms_and_cond'] != "") {
            $rep->NewLine();
            $rep->TextCol(1, 4, "  TERMS & CONDITIONS");
            $rep->NewLine();
            $rep->TextCol(1, 4, "  ..................................");
            $rep->NewLine();
            $rep->NewLine();
            $rep->TextColLines(1, 4, $myrow['terms_and_cond'], -3, 0, 0, 0, null, 0, 7);
        }
        
        if ($myrow['pay_terms'] != "") {
            $rep->NewLine();
            $rep->TextCol(1, 4, "  PAYMENT TERMS");
            $rep->NewLine();
            $rep->TextCol(1, 4, "  ..................................");
            $rep->NewLine();
            $rep->NewLine();
            $rep->TextColLines(1, 4, $myrow['pay_terms'], -3, 0, 0, 0, null, 0, 7);
        }
        ...
        $rep->TextCol(4, 5, trans("Sub-total"), -2);
        ...
        foreach ($tax_items as $tax_item) {
            ...
            if ($myrow['tax_included']) {
                if ($SysPrefs->alternative_tax_include_on_docs() == 1) {
                    if ($first) {
                        $rep->TextCol(3, 6, trans("Total Tax Excluded"), -2);
                    ...
                    $rep->TextCol(3, 7, trans("Included") . " " . $tax_type_name . trans("Amount") . ": " . $DisplayTax, -2);
            } else {
        ...
        $rep->Font('bold');
        $rep->TextCol(4, 5, trans("TOTAL PO"), -2);
