								...
								$tags, $thousands)
{
	...
	if ($printtitle)
	{
		...
		$rep->Line($rep->row);
		$rep->NewLine();
		$rep->TextCol(0, 2,	trans('Total') . " " . $typename);
		for ($i = 1; $i <= 13; $i++)
...
function print_annual_expense_breakdown()
{
	...
	if ($thousands)
	{
		$dec = 1;
		$thousands = 1000;
		$amts_thousands = trans("Amounts in thousands");
		$fontSize = 9;
	}
	else
	{
	...
	$headers = array(trans('Account'), trans('Account Name'), $per01, $per02, $per03, $per04,
		$per05, $per06, $per07, $per08, $per09, $per10, $per11, $per12, trans('Total'));
    ...
    if ($dim == 2)
    {
    	$params =   array( 	0 => $comments,
                    	1 => array('text' => trans("Year"),
                    		'from' => $year, 'to' => ''),
                    	2 => array('text' => trans("Dimension")." 1",
                    		'from' => get_dimension_string($dimension), 'to' => ''),
                    	3 => array('text' => trans("Dimension")." 2",
                    		'from' => get_dimension_string($dimension2), 'to' => ''),
                    	4 => array('text' => trans('Tags'), 'from' => get_tag_names($tags), 'to' => ''),	
                    	5 => array('text' => trans('Info'), 'from' => $amts_thousands, 'to' => ''));
    }
    elseif ($dim == 1)
    {
    	$params =   array( 	0 => $comments,
                    	1 => array('text' => trans("Year"),
                    		'from' => $year, 'to' => ''),
                    	2 => array('text' => trans('Dimension'),
                    		'from' => get_dimension_string($dimension), 'to' => ''),
                    	3 => array('text' => trans('Tags'), 'from' => get_tag_names($tags), 'to' => ''),	
                    	4 => array('text' => trans('Info'), 'from' => $amts_thousands, 'to' => ''));
    }
    else
    {
    	$params =   array( 	0 => $comments,
                    	1 => array('text' => trans("Year"),
                    		'from' => $year, 'to' => ''),
                    	2 => array('text' => trans('Tags'), 'from' => get_tag_names($tags), 'to' => ''),	
                    	3 => array('text' => trans('Info'), 'from' => $amts_thousands, 'to' => ''));
    }

	$rep = new FrontReport(trans('Annual Expense Breakdown'), "AnnualBreakDown", user_pagesize(), $fontSize,
										$orientation);
    if ($orientation == 'L')
	...
	while ($class = db_fetch($classresult))
	{
		...
		$rep->NewLine();
		$rep->Font('bold');
		$rep->TextCol(0, 2,	trans('Total') . " " . $class["class_name"]);
		for ($i = 1; $i <= 13; $i++)
		{
	...
	$rep->Font('bold');	
	$rep->TextCol(0, 2,	trans("Calculated Return"));
	for ($i = 1; $i <= 13; $i++)
