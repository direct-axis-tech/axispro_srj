...
function print_workorders()
{
	...
	if ($email == 0)
		$rep = new FrontReport(trans('WORK ORDER'), "WorkOrderBulk", user_pagesize(), 9, $orientation);
   	if ($orientation == 'L')
	...
	for ($i = $from; $i <= $to; $i++)
	{
		...
		$rep->TextCol(0, 5,trans("Work Order Requirements"), -2);
		$rep->NewLine(2);
