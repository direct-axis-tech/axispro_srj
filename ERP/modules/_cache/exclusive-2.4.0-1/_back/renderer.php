...
include_once('xpMenu.class.php');
	...
	class renderer
	{
		...
		function menu_header($title, $no_menu, $is_index)
		{
			...
			if (!$no_menu)
			{
				...
				$img = "<img src='$local_path_to_root/themes/exclusive/images/login.gif' width='14' height='14' border='0' alt='".trans('Logout')."'>&nbsp;&nbsp;";
				$himg = "<img src='$local_path_to_root/themes/exclusive/images/help.gif' width='14' height='14' border='0' alt='".trans('Help')."'>&nbsp;&nbsp;";
				echo "<div id='header'>\n";
				echo "<ul>\n";
				echo "  <li><a href='$path_to_root/admin/display_prefs.php?'>" . trans("Preferences") . "</a></li>\n";
				echo "  <li><a href='$path_to_root/admin/change_current_user_password.php?selected_id=" . $_SESSION["wa_current_user"]->username . "'>" . trans("Change password") . "</a></li>\n";
			 	if ($help_base_url != null)
					echo "  <li><a target = '_blank' onclick=" .'"'."javascript:openWindow(this.href,this.target); return false;".'" '. "href='". 
						help_url()."'>$himg" . trans("Help") . "</a></li>";
				echo "  <li><a href='$path_to_root/access/logout.php?'>$img" . trans("Logout") . "</a></li>";
				echo "</ul>\n";
		...
		function menu_footer($no_menu, $is_index)
		{
			...
			if (!$no_menu)
			{
				...
				if (isset($_SESSION['wa_current_user']))
				{
					...
					echo "<span class='date'>".Today() . "&nbsp;" . CurrentTime()."</span>\n";
					...
					echo "<span class='date'>" . trans("Theme:") . " " . user_theme() . "</span>\n";
	...
	function display_customer_topten()
	{
		...
		$sql = "SELECT SUM((ov_amount + ov_discount) * rate * IF(trans.type = ".ST_CUSTCREDIT.", -1, 1)) AS total,d.debtor_no, d.name FROM
			...
			LIMIT 10";
		$result = db_query($sql);
		$title = trans("Top 10 customers in fiscal year");
		br(2);
		display_heading($title);
		br();
		$th = array(trans("Customer"), trans("Amount"));
		...
		$result = db_query($sql);
		$title = db_num_rows($result) . trans(" overdue Sales Invoices");
		br(1);
		display_heading($title);
		br();
		$th = array("#", trans("Ref."), trans("Date"), trans("Due Date"), trans("Customer"), trans("Branch"), trans("Currency"),
			trans("Total"), trans("Remainder"),	trans("Days"));
		start_table(TABLESTYLE);
		table_header($th);
		$k = 0; //row colour counter
		while ($myrow = db_fetch($result))
	...
	function display_supplier_topten()
	{
		...
		$sql = "SELECT SUM((trans.ov_amount + trans.ov_discount) * rate) AS total, s.supplier_id, s.supp_name FROM
			...
			LIMIT 10";
		$result = db_query($sql);
		$title = trans("Top 10 suppliers in fiscal year");
		br(2);
		display_heading($title);
		br();
		$th = array(trans("Supplier"), trans("Amount"));
		start_table(TABLESTYLE, "width=30%");
		table_header($th);
		$k = 0; //row colour counter
		$i = 0;
		...
		$sql = "SELECT trans.trans_no, trans.reference, trans.tran_date, trans.due_date, s.supplier_id, 
			...
			WHERE s.supplier_id = trans.supplier_id
				...
				AND DATEDIFF('$today', trans.due_date) > 0 ORDER BY days DESC";
		$result = db_query($sql);
		$title = db_num_rows($result) . trans(" overdue Purchase Invoices");
		br(1);
		display_heading($title);
		br();
		$th = array("#", trans("Ref."), trans("Date"), trans("Due Date"), trans("Supplier"), trans("Currency"), trans("Total"),
			trans("Remainder"),	trans("Days"));
		start_table(TABLESTYLE);
		table_header($th);
		$k = 0; //row colour counter
		while ($myrow = db_fetch($result))
	...
	function display_stock_topten($manuf=false)
	{
		...
		$result = db_query($sql);
		if ($manuf)
			$title = trans("Top 10 Manufactured Items in fiscal year");
		else	
			$title = trans("Top 10 Sold Items in fiscal year");
		br(2);
		display_heading($title);
		br();
	...
	function display_dimension_topten()
	{
		...
		$title = trans("Top 10 Dimensions in fiscal year");
		br(2);
		display_heading($title);
		br();
		$th = array(trans("Dimension"), trans("Amount"));
		start_table(TABLESTYLE, "width=30%");
		table_header($th);
		$k = 0; //row colour counter
		$i = 0;
