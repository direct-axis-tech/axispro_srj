...
$path_to_root = '..';
$page_security = 'SA_LEAVE_ACCRUALS';

require_once $path_to_root . "/includes/session.inc";
require_once $path_to_root . "/hrm/helpers/leaveAccrualHelpers.php";

$inputs = LeaveAccrualHelpers::getValidatedInputs();

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (!user_check_access('SA_LEAVE_ACCRUALS')) {
    ...
    if (!empty($_POST['action'])) {
        if ($_POST['action'] == 'showDetails') {
            LeaveAccrualHelpers::handleShowDetailsRequest($inputs);
            exit();
        }
        
        if ($_POST['action'] == 'postGL') {
            LeaveAccrualHelpers::handlePostGLRequest($inputs);
            exit();
        }
    }
...
page(trans('Process Leave Accruals'), false, false, '', '', false, '', true);

if (empty(pref('hr.leave_accrual_payable_account')) || empty(pref('hr.leave_accrual_expense_account'))) {
    echo '<div class="text-center w-100 py-10 text-danger fs-1">Please configure the leave accrual accounts</div>';
    end_page();
    exit();
}
...
<div id="_content" class="text-dark">
    <div class="card mx-5">
        <form
            ...
            action=""
            class="w-100"
            id="leave-accrual-form">
            <div class="card mx-auto mb-3 mt-5 mw-800px">
                <div class="card-header">
                    <h1 class="card-title text-gray-600">Leave Accruals</h1>
                </div>
                <div class="card-body">
                    <div class="form-group row mt-5">
                        ...
                        <div class="col-sm-9">
                            <select
                                ...
                                name="employee_ids[]" id="employee_id">
                    ...
                    <div class="form-group row required">
                        <label for="as_of_date" class="col-form-label col-sm-3">Leave accruals as of:</label>
                        <div class="col-sm-9">
                            <input
                                type="text"
                                required
                                ...
                                value="<?= sql2date($inputs['as_of_date']) ?>">
                            <small id="as_of_date_help" class="form-text text-muted">The date till which the leave accruals are calculated</small>
                        </div>
                    </div>
                    ...
                    <div class="form-group row required">
                        ...
                        <div class="col-sm-9">
                            <input
                                ...
                                data-date-format="<?= getDateFormatForBSDatepicker() ?>"
                                data-date-autoclose="true"
                                value="<?= sql2date($inputs['trans_date']) ?>">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <textarea
                                ...
                                data-parsley-pattern2-message="The name must only contains alphabets, numbers, dashes, underscore, comma, period, slash or spaces"
                                class="form-control"
                                id="memo">Leave accrual as of <?= sql2date($inputs['as_of_date']) ?></textarea>
                        </div>
                    </div>
                </div>
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Leave Accrual Details As Of <span data-as-of></span></h5>
            </div>
...
$(function() {
    ...
    var parsleyForm = $('#leave-accrual-form').parsley({
    ...
    $('#as_of_date').on('change', function () {
        ...
        if (date.isValid()) {
            $('#memo').val(`Leave accrual as of ${this.value}`);
    ...
    function renderModal() {
        const dataTable = $('#detailsDisplayTable').DataTable({
            ...
            columns: [
                {data: 'employee_ref', width: '300px', className: 'ps-3 text-wrap text-start'},
                {data: 'employee_name', className: 'text-start'},
                {data: 'date_of_join', render: formatters.date, className: 'text-center'},
                {data: 'leave_salary', render: formatters.number},
                {data: 'work_days'},
                {data: 'available_leaves'},
                {data: 'last_accrual_on', render: formatters.date},
                {data: 'accumulated_amount', render: formatters.number},
                {data: 'accrued_amount', render: formatters.number},
                {data: 'this_postings', render: formatters.number, className: 'pe-3'},
            ]
        })
