...
//	if (trigger.type=='submit' && !validate(trigger)) return false;
	tout = tout || 30000;	// default timeout value
	document.getElementById('msgbox').innerHTML='';
	set_mark(tout>30000 ? 'progressbar.gif' : 'ajax-loader.gif');
            ...
            if (result) {
              for (var i in result) {
                ...
                if (cmd == "as") {
                  ...
                  if (objElement) {
                    if (
                      objElement.tagName == "INPUT" ||
                      objElement.tagName == "TEXTAREA"
                    )
                      objElement.value = data;
                    else objElement.innerHTML = data; // selector, div, span etc
                  }
                } else if (cmd == "di") {
                  ...
                  window.open(
                    data,
                    "REP_WINDOW",
                    "toolbar=no,scrollbars=yes,resizable=yes,menubar=no"
                  );
                } else {
                  errors = errors + "<br>Unknown ajax function: " + cmd;
                }
              }
              if (tcheck) JsHttpRequest.clearTimeout(tcheck);
