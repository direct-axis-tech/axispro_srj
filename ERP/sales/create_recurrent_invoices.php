...
page(trans($help_context = "Create and Print Recurrent Invoices"), false, false, "", $js);
...
if ($id != -1)
{
	...
	*/
	...
	if (count($invs) > 0)
	{
		...
		$min = $max = 0;
	display_notification(sprintf(trans("%s recurrent invoice(s) created, # %s - # %s."), count($invs), $min, $max));
	if (count($invs) > 0)
	{
		...
		display_note(print_link(sprintf(trans("&Print Recurrent Invoices # %s - # %s"), $min, $max), 107, $ar), 0, 1);
		$ar['PARAM_3'] = 1; // email
		display_note(print_link(sprintf(trans("&Email Recurrent Invoices # %s - # %s"), $min, $max), 107, $ar), 0, 1);
	}
}
...
if ($id != -1)
{
	...
	elseif (!date1_greater_date2(add_days(Today(), 1), $to))
		display_error(trans("Recurrent invoice cannot be generated before last day of covered period."));
	elseif (check_recurrent_invoice_prices($id))
		display_error(trans("Recurrent invoices cannot be generated because some items have no price defined in customer currency."));
	elseif (!check_sales_order_type($myrow['order_no']))
		display_error(trans("Recurrent invoices cannot be generated because selected sales order template uses prepayment sales terms. Change payment terms and try again."));
	else {
		...
		label_row(trans('Description:'), $myrow["description"]);
		label_row(trans('Template:'), get_customer_trans_view_str(ST_SALESORDER, $myrow["order_no"]));
		label_row(trans('Number of invoices:'), $count);
		date_row(trans('Invoice date:'), 'trans_date');
		$newto = add_months($to, $myrow['monthly']);
		$newto = add_days($newto, $myrow['days']);
		text_row(trans('Invoice notice:'), 'memo', sprintf(trans("Recurrent Invoice covers period %s - %s."), $to,	 add_days($newto, -1)), 100, 100);
		//text_row(trans('Invoice notice:'), 'memo', sprintf(trans("Recurrent Invoice covers period %s - %s."), //$from, add_days($to, -1)), 100, 100);
		end_table();
		hidden('from', $from, true);
		hidden('to', $to, true);
		br();
		submit_center_first('confirmed'.$id, trans('Create'), trans('Create recurrent invoices'), false, ICON_OK);
		submit_center_last('cancel', trans('Cancel'), trans('Return to recurrent invoices'), false, ICON_ESCAPE);
		submit_js_confirm("do_create".$id, sprintf(trans("You are about to issue %s invoices.\n Do you want to continue?"), $count));
		end_form();
...
$th = array(trans("Description"), trans("Template No"),trans("Customer"),trans("Branch")."/".trans("Group"),trans("Days"),trans("Monthly"),trans("Begin"),trans("End"),trans("Next invoice"),"");
table_header($th);
$k = 0;
$due = false;
while ($myrow = db_fetch($result)) 
{
	...
	if ($myrow['overdue'])
	{
		$count = recurrent_invoice_count($myrow['id']);
 		if ($count)
 		{
			button_cell("create".$myrow["id"], sprintf(trans("Create %s Invoice(s)"), $count), "", ICON_DOC, 'process');
		} else {
			label_cell('');
		}
	}
...
end_form();
if ($due)
	display_note(trans("Marked items are due."), 1, 0, "class='overduefg'");
else
	display_note(trans("No recurrent invoices are due."), 1, 0);
