...
page(trans($help_context = "Customer Branches"), @$_REQUEST['popup'], false, "", $js);
...
check_db_has_customers(trans("There are no customers defined in the system. Please define a customer to add customer branches."));

check_db_has_sales_people(trans("There are no sales people defined in the system. At least one sales person is required before proceeding."));

check_db_has_sales_areas(trans("There are no sales areas defined in the system. At least one sales area is required before proceeding."));

check_db_has_shippers(trans("There are no shipping companies defined in the system. At least one shipping company is required before proceeding."));
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM')
{
	...
	{
		$input_error = 1;
		display_error(trans("The Branch name cannot be empty."));
		set_focus('br_name');
	}
	...
	{
		$input_error = 1;
		display_error(trans("The Branch short name cannot be empty."));
		set_focus('br_ref');
	}
	...
	if ($input_error != 1)
	{
    	...
    	if ($selected_id != -1)
		{
			...
			$note =trans('Selected customer branch has been updated');
  		}
		else
		{
			...
			$note = trans('New customer branch has been added');
		}
		commit_transaction();
...
elseif ($Mode == 'Delete')
{
	...
	{
		display_error(trans("Cannot delete this branch because customer transactions have been created to this branch."));
	...
	else
	{
		...
		{
			display_error(trans("Cannot delete this branch because sales orders exist for it. Purge old sales orders first."));
		}
		else
		{
			delete_branch($_POST['customer_id'], $_POST['branch_code']);
			display_notification(trans('Selected customer branch has been deleted'));
		}
	}
	$Mode = 'RESET';
}
...
function edit_link($row) {
	return button("Edit".$row["branch_code"],trans("Edit"), '', ICON_EDIT);
}

function del_link($row) {
	return button("Delete".$row["branch_code"],trans("Delete"), '', ICON_DELETE);
}
...
function branch_settings($selected_id, $num_branches) {
	...
	elseif ($Mode != 'ADD_ITEM')
	{
		...
		if($myrow && !$num_branches) {
			...
			$_POST['contact_name'] = trans('Main Branch');
	...
	table_section_title(trans("Name and Contact"));
	text_row(trans("Branch Name:"), 'br_name', null, 50, 60);
	if ($selected_id != -1) {
		label_row('Branch Short Name:', $_POST['br_ref']);
		hidden('br_ref');
	} else {
		text_row(trans("Branch Short Name:"), 'br_ref', null, 30, 30);
	}
	...
	sales_groups_list_row(trans("Sales Group:"), 'group_no', null, true);
	locations_list_row(trans("Default Inventory Location:"), 'default_location', null);
	shippers_list_row(trans("Default Shipping Company:"), 'default_ship_via', null);
	tax_groups_list_row(trans("Tax Group:"), 'tax_group_id', null);

	table_section_title(trans("GL Accounts"));

	gl_all_accounts_list_row(trans("Sales Account:"), 'sales_account', null, false, false, true);
	gl_all_accounts_list_row(trans("Sales Discount Account:"), 'sales_discount_account');
	gl_all_accounts_list_row(trans("Accounts Receivable Account:"), 'receivables_account', null, true);
	gl_all_accounts_list_row(trans("Prompt Payment Discount Account:"), 'payment_discount_account');
	text_row(trans("Bank Account Number:"), 'bank_account', null, 30, 60);

	table_section(2);

	if($selected_id==-1) {
		table_section_title(trans("General contact data"));
		text_row(trans("Contact Person:"), 'contact_name', null, 35, 40);
		...
		languages_list_row( trans("Document Language:"), 'rep_lang', null, trans("Customer default"));
	}
	table_section_title(trans("Addresses"));
	textarea_row(trans("Mailing Address:"), 'br_post_address', null, 35, 4);
	textarea_row(trans("Billing Address:"), 'br_address', null, 35, 4);
	textarea_row(trans("General Notes:"), 'notes', null, 35, 4);
...
echo "<center>" . trans("Select a customer: ") . "&nbsp;&nbsp;";
...
display_db_pager($table);
	...
	display_note(trans("The selected customer does not have any branches. Please create at least one branch."));

tabbed_content_start('tabs', array(
		'settings' => array(trans('&General settings'), $selected_id!=-1),
		'contacts' => array(trans('&Contacts'), $selected_id!=-1),
