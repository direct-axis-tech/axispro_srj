...
if (isset($_GET['ModifyDelivery'])) {
	$_SESSION['page_title'] = sprintf(trans("Modifying Delivery Note # %d."), $_GET['ModifyDelivery']);
	$help_context = "Modifying Delivery Note";
	processing_start();
} elseif (isset($_GET['OrderNumber'])) {
	$_SESSION['page_title'] = trans($help_context = "Deliver Items for a Sales Order");
	processing_start();
}
...
if (isset($_GET['AddedID'])) {
	...
	display_notification_centered(sprintf(trans("Delivery # %d has been entered."),$dispatch_no));

	display_note(get_customer_trans_view_str(ST_CUSTDELIVERY, $dispatch_no, trans("&View This Delivery")), 0, 1);

	display_note(print_document_link($dispatch_no, trans("&Print Delivery Note"), true, ST_CUSTDELIVERY));
	display_note(print_document_link($dispatch_no, trans("&Email Delivery Note"), true, ST_CUSTDELIVERY, false, "printlink", "", 1), 1, 1);
	display_note(print_document_link($dispatch_no, trans("P&rint as Packing Slip"), true, ST_CUSTDELIVERY, false, "printlink", "", 0, 1));
	display_note(print_document_link($dispatch_no, trans("E&mail as Packing Slip"), true, ST_CUSTDELIVERY, false, "printlink", "", 1, 1), 1);

	display_note(get_gl_view_str(13, $dispatch_no, trans("View the GL Journal Entries for this Dispatch")),1);

	if (!isset($_GET['prepaid']))
		hyperlink_params("$path_to_root/sales/customer_invoice.php", trans("Invoice This Delivery"), "DeliveryNumber=$dispatch_no");

	hyperlink_params("$path_to_root/sales/inquiry/sales_orders_view.php", trans("Select Another Order For Dispatch"), "OutstandingOnly=1");
	...
	display_notification_centered(sprintf(trans('Delivery Note # %d has been updated.'),$delivery_no));

	display_note(get_trans_view_str(ST_CUSTDELIVERY, $delivery_no, trans("View this delivery")), 0, 1);

	display_note(print_document_link($delivery_no, trans("&Print Delivery Note"), true, ST_CUSTDELIVERY));
	display_note(print_document_link($delivery_no, trans("&Email Delivery Note"), true, ST_CUSTDELIVERY, false, "printlink", "", 1), 1, 1);
	display_note(print_document_link($delivery_no, trans("P&rint as Packing Slip"), true, ST_CUSTDELIVERY, false, "printlink", "", 0, 1));
	display_note(print_document_link($delivery_no, trans("E&mail as Packing Slip"), true, ST_CUSTDELIVERY, false, "printlink", "", 1, 1), 1);

	if (!isset($_GET['prepaid']))
		hyperlink_params($path_to_root . "/sales/customer_invoice.php", trans("Confirm Delivery and Invoice"), "DeliveryNumber=$delivery_no");

	hyperlink_params($path_to_root . "/sales/inquiry/sales_deliveries_view.php", trans("Select A Different Delivery"), "OutstandingOnly=1");
...
if (isset($_GET['OrderNumber']) && $_GET['OrderNumber'] > 0) {
	...
	if ($ord->count_items() == 0) {
		hyperlink_params($path_to_root . "/sales/inquiry/sales_orders_view.php",
			trans("Select a different sales order to delivery"), "OutstandingOnly=1");
		echo "<br><center><b>" . trans("This order has no items. There is nothing to delivery.") .
			"</center></b>";
		display_footer_exit();
	} else if (!$ord->is_released()) {
		hyperlink_params($path_to_root . "/sales/inquiry/sales_orders_view.php",trans("Select a different sales order to delivery"),
			"OutstandingOnly=1");
		echo "<br><center><b>".trans("This prepayment order is not yet ready for delivery due to insufficient amount received.")
			."</center></b>";
		display_footer_exit();
	}
	...
	if (!$_SESSION['Items']->prepaid && $_SESSION['Items']->count_items() == 0) {
		hyperlink_params($path_to_root . "/sales/inquiry/sales_orders_view.php",
			trans("Select a different delivery"), "OutstandingOnly=1");
		echo "<br><center><b>" . trans("This delivery has all items invoiced. There is nothing to modify.") .
			"</center></b>";
		display_footer_exit();
	}
	...
	display_error(trans("This page can only be opened if an order or delivery note has been selected. Please select it first."));

	hyperlink_params("$path_to_root/sales/inquiry/sales_orders_view.php", trans("Select a Sales Order to Delivery"), "OutstandingOnly=1");
	...
	if (!check_quantities()) {
		display_error(trans("Selected quantity cannot be less than quantity invoiced nor more than quantity	not dispatched on sales order."));

	} elseif(!check_num('ChargeFreightCost', 0)) {
		display_error(trans("Freight cost cannot be less than zero"));
...
function check_data()
{
	...
	if (!isset($_POST['DispatchDate']) || !is_date($_POST['DispatchDate']))	{
		display_error(trans("The entered date of delivery is invalid."));
	...
	if (!isset($_POST['due_date']) || !is_date($_POST['due_date']))	{
		display_error(trans("The entered dead-line for invoice is invalid."));
		set_focus('due_date');
		return false;
	}
	...
	if ($_SESSION['Items']->has_items_dispatch() == 0 && input_num('ChargeFreightCost') == 0) {
		display_error(trans("There are no item quantities on this delivery note."));
		return false;
	}
...
if (isset($_POST['process_delivery']) && check_data()) {
	process_cart($_SESSION['Items'], function (&$dn) {
		if ($_POST['bo_policy']) {
			$bo_policy = 0;
		} else {
			$bo_policy = 1;
...
if ($_SESSION['Items']->trans_no==0) {
	ref_cells(trans("Reference"), 'ref', '', null, "class='tableheader2'", false, ST_CUSTDELIVERY,
...
label_cells(trans("For Sales Order"), get_customer_trans_view_str(ST_SALESORDER, $_SESSION['Items']->order_no), "class='tableheader2'");
...
label_cell(trans("Delivery From"), "class='tableheader2'");
...
date_cells(trans("Date"), 'DispatchDate', '', $_SESSION['Items']->trans_no==0, 0, 0, 0, "class='tableheader2'");
end_row();
...
//---------
start_row();
date_cells(trans("Invoice Dead-line"), 'due_date', '', null, 0, 0, 0, "class='tableheader2'");
end_row();
end_table();
...
display_heading(trans("Delivery Items"));
div_start('Items');
start_table(TABLESTYLE, "width='80%'");

$new = $_SESSION['Items']->trans_no==0;
$th = array(trans("Item Code"), trans("Item Description"), 
	$new ? trans("Ordered") : trans("Max. delivery"), trans("Units"), $new ? trans("Delivered") : trans("Invoiced"),
	trans("This Delivery"), trans("Price"), trans("Tax Type"), trans("Discount"), trans("Total"));
...
foreach ($_SESSION['Items']->line_items as $line=>$ln_itm) {
	...
	if (has_stock_holding($ln_itm->mb_flag) && $ln_itm->qty_dispatched) {
		...
		if ($check = check_negative_stock($ln_itm->stock_id, $ln_itm->qty_done-$ln_itm->qty_dispatched, $_POST['Location'], $_POST['DispatchDate'], null, $_SESSION['Items']->contract ? $_SESSION['Items']->contract->labour_id: null))
			$qty = $check['qty'];
...
label_row(trans("Sub-total"), $display_sub_total, "colspan=$colspan align=right","align=right");
...
label_row(trans("Amount Total"), $display_total, "colspan=$colspan align=right","align=right");
...
policy_list_row(trans("Action For Balance"), "bo_policy", null);
...
if(isset($_POST['clear_quantity'])) {
	submit('reset_quantity', trans('Reset quantity'), true, trans('Refresh document page'));
}
else  {
	submit('clear_quantity', trans('Clear quantity'), true, trans('Refresh document page'));
}
submit_center_last('process_delivery', trans("Process Dispatch"),
