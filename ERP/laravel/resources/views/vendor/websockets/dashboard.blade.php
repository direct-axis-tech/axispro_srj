...
<script src="<?= asset(mix('js/utils.bundle.js')) ?>"></script>
<script>
    new Vue({
        el: '#app',
        ...
        methods: {
            connect() {
                this.pusher = new Pusher(this.app.key, {
                    ...
                    disableStats: true,
                    authEndpoint: url('/v3/{{ request()->path() }}/auth'),
                    auth: {
                        headers: {
            ...
            loadChart() {
                $.getJSON(url('/v3/{{ request()->path() }}/api/'+this.app.id+'/statistics'), (data) => {
            ...
            sendEvent() {
                $.post(url('/v3/{{ request()->path() }}/event'), {
                    _token: '{{ csrf_token() }}',
                    key: this.app.key,
