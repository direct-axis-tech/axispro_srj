...
class AddTransDatesColumnsToSuppAllocationsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        if (!Schema::hasColumn('0_supp_allocations', 'date_alloc_to')) {
        ...
        try {
            DB::table('0_supp_allocations as alloc')
                ->leftJoin('0_supp_trans as t_to', function (JoinClause $join) {
                    $join->on('t_to.supplier_id', 'alloc.person_id')
                        ->whereColumn('t_to.type', 'alloc.trans_type_to')
                        ->whereColumn('t_to.trans_no', 'alloc.trans_no_to');
                })
                ->leftJoin('0_purch_orders as o_to', function (JoinClause $join) {
                    $join->on('o_to.supplier_id', 'alloc.person_id')
                    ->whereColumn('o_to.order_no', 'alloc.trans_no_to')
                    ->where('alloc.trans_type_to', ST_PURCHORDER);
                })
                ->leftJoin('0_supp_trans as t_from', function (JoinClause $join) {
                    $join->on('t_from.supplier_id', 'alloc.person_id')
