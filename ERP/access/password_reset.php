...
***********************************************************************/
    ...
    include_once($path_to_root . "/themes/daxis/password_reset.php");
...
function defaultCompany()
{
	...
	$title = $SysPrefs->app_title." ".$version." - ".trans("Password reset");
	...
	echo "<link href='$path_to_root/themes/$def_theme/default.css?id=v1.0.4' rel='stylesheet' type='text/css'> \n";
	...
	table_section_title(trans("Version")." $version   Build ".$SysPrefs->build_version." - ".trans("Password reset"));

	text_row(trans("Email"), "email_entry_field", "", 20, 30);

    $coy =  user_company();
    if (!isset($coy))
        $coy = $def_coy;
    if (!@$SysPrefs->text_company_selection) {
        echo "<tr><td>".trans("Company")."</td><td><select name='company_login_name'>\n";
        for ($i = 0; $i < count($db_connections); $i++)
            echo "<option value=$i ".($i==$coy ? 'selected':'') .">" . $db_connections[$i]["name"] . "</option>";
        echo "</select>\n";
        echo "</td></tr>";
    } else {
        text_row(trans("Company"), "company_login_nickname", "", 20, 50);
    }
    start_row();
    label_cell("Please enter your e-mail", "colspan=2 align='center' id='log_msg'");
    end_row();
	end_table(1);
	echo "<center><input type='submit' value='&nbsp;&nbsp;".trans("Send password -->")."&nbsp;&nbsp;' name='SubmitReset'
	...
	echo "<tr>\n";
	echo "<td><a target='_blank' href='".$SysPrefs->power_url."' tabindex='-1'>".$SysPrefs->app_title." $version - " . trans("Theme:") . " " . $def_theme . "</a></td>\n";
	echo "</tr>\n";
	echo "<tr>\n";
