...
page(trans($help_context = "Tax Groups"));
...
check_db_has_tax_types(trans("There are no tax types defined. Define tax types before defining tax groups."));
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	{
		$input_error = 1;
		display_error(trans("The tax group name cannot be empty."));
		set_focus('name');
	} 
	if ($input_error != 1) 
	{
    	...
    	if ($selected_id != -1) 
    	{
			...
			display_notification(trans('Selected tax group has been updated'));
    	} 
    	else 
    	{
	   		add_tax_group($_POST['name'], $taxes, $tax_shippings);
			display_notification(trans('New tax group has been added'));
    	}
...
function can_delete($selected_id)
{
	...
	{
		display_error(trans("Cannot delete this tax group because customer branches been created referring to it."));
		return false;
	}

	if (key_in_foreign_table($selected_id, 'suppliers', 'tax_group_id'))
	{
		display_error(trans("Cannot delete this tax group because suppliers been created referring to it."));
		return false;
	}
...
if ($Mode == 'Delete')
{
	...
	if (can_delete($selected_id))
	{
		...
		display_notification(trans('Selected tax group has been deleted'));
	}
	$Mode = 'RESET';
}
...
text_row_ex(trans("Description:"), 'name', 40);

end_table();

display_note(trans("Select the taxes that are included in this group."), 1, 1);

$items = get_tax_group_rates($selected_id!=-1 ? $selected_id : null);

start_table(TABLESTYLE2);
$th = array(trans("Tax"), "", trans("Shipping Tax"));
table_header($th);
