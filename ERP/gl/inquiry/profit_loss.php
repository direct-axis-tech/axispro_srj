...
page(trans($help_context = "Profit & Loss Drilldown"), false, false, "", $js);
	...
	$dimension=0, $dimension2=0, $drilldown, $path_to_root)
{
	...
	if (($code_per_balance + $per_balance_total + $code_acc_balance + $acc_balance_total) != 0)
	{
		...
		{		
			...
			label_cell(trans('Total') . " " . $typename);
...
function display_profit_and_loss($compare)
{
	...
	$tableheader =  "<tr>
        <td class='tableheader'>" . trans("Group/Account Name") . "</td>
        <td class='tableheader'>" . trans("Period") . "</td>
		<td class='tableheader'>" . $compare_types[$compare] . "</td>
		<td class='tableheader'>" . trans("Achieved %") . "</td>
        </tr>";	
	...
	if (!$drilldown) //Root Level
	{
		...
		while ($class = db_fetch($classresult))
		{
			...
			start_row("class='inquirybg' style='font-weight:bold'");
			label_cell(trans('Total') . " " . $class["class_name"]);
		...
		start_row("class='inquirybg' style='font-weight:bold'");
		label_cell(trans('Calculated Return'));
