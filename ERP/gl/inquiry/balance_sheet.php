...
page(trans($help_context = "Balance Sheet Drilldown"), false, false, "", $js);
...
function inquiry_controls()
{
	...
	date_cells(trans("As at:"), 'TransToDate');
	if ($dim >= 1)
...
function display_balance_sheet()
{
	...
	{		
		...
		$url = "<a href='$path_to_root/gl/inquiry/profit_loss.php?TransFromDate=" 
				. $from."&TransToDate=".$to . "&Dimension=" . $dimension . "&Dimension2=" . $dimension2
			."&Compare=0'>".trans('Calculated Return')."</a>";		
		...
		start_row("class='inquirybg' style='font-weight:bold'");
		label_cell(trans('Total') . " " . trans('Liabilities') . trans(' and ') . trans('Equities'));
