...
if (get_company_pref('use_dimension') && isset($_POST['dimension'])) // display dimension only, when started in dimension mode
    dimensions_list_cells(_('Projects:'), 'dimension', null, true, null, true);
...
{
   return get_trans_view_str($row["trans_type"], $row["trans_no"], $row['reference']);
}
...
function edit_link($row)
{
    ...
    if ($row['trans_type'] == ST_CUSTPAYMENT) {
        $ok = user_check_access('SA_MODSALESPAYMENT');
    }

    if (strpos($row['memo_'], 'Fixed asset has been deprecated by the value of') === 0) {
        $ok = false;
    }

    if (strpos($row['memo_'], 'Employee Deductions Applied of') === 0) {
        $ok = false;
    }

    if ($row['trans_type'] == ST_CUSTCREDIT) {
        $myrow = get_customer_trans($row["trans_no"], $row["trans_type"]);
        if(!empty($myrow['contract_id']))
            $ok = false;
    }
...
function prt_link($row)
{
    ...
    if(in_array($row['trans_type'],[0,1,2])){
        $url=$path_to_root.'/voucher_print?voucher_id='.$row["trans_no"].'-'.$row["trans_type"];
