...
page(trans($help_context = "Trial Balance"), false, false, "", $js);
...
function gl_inquiry_controls()
{
    ...
    start_row();
    date_cells(trans("From:"), 'TransFromDate');
    ...
    check_cells(trans("No zero values"), 'NoZero', null);
    check_cells(trans(""), 'Balance', 1,false,'',"style='display:none'");
	check_cells(trans("Group totals only"), 'GroupTotalOnly', null);
...
function display_trial_balance($type, $typename)
{
    ...
    while ($account = db_fetch($accounts))
    {
        ...
        if (!$printtitle)
        {
			if (!check_value('GroupTotalOnly'))
			{
				...
				label_cell(trans("Group")." - ".$type ." - ".$typename, "colspan=8");
				end_row();
			}
            $printtitle = 1;
        }
    ...
    start_row("class='inquirybg' style='font-weight:bold'");
	if (!check_value('GroupTotalOnly'))
		label_cell(trans("Total") ." - ".$typename, "colspan=2");
	else
...
if (isset($_POST['TransFromDate']))
{
    ...
    {
        display_error(trans("The from date cannot be bigger than the fiscal year end."));
...
$tableheader =  "<tr>
	<td rowspan=2 class='tableheader'>" . trans("Account") . "</td>
	<td rowspan=2 class='tableheader'>" . trans("Account Name") . "</td>
	<td colspan=2 class='tableheader'>" . trans("Brought Forward") . "</td>
	<td colspan=2 class='tableheader'>" . trans("This Period") . "</td>
	<td colspan=2 class='tableheader'>" . trans("Balance") . "</td>
	</tr><tr>
	<td class='tableheader'>" . trans("Debit") . "</td>
	<td class='tableheader'>" . trans("Credit") . "</td>
	<td class='tableheader'>" . trans("Debit") . "</td>
	<td class='tableheader'>" . trans("Credit") . "</td>
	<td class='tableheader'>" . trans("Debit") . "</td>
	<td class='tableheader'>" . trans("Credit") . "</td>
...
{
    start_row("class='inquirybg' style='font-weight:bold'");
    label_cell(trans("Class")." - ".$class['cid'] ." - ".$class['class_name'], "colspan=8");
    end_row();
...
{
    start_row("class='inquirybg' style='font-weight:bold'");
    label_cell(trans("Total") ." - ".$_POST['TransToDate'], "colspan=2");
    amount_cell($pdeb);
    amount_cell($pcre);
    amount_cell($cdeb);
    amount_cell($ccre);
...
start_row("class='inquirybg' style='font-weight:bold'");
label_cell(trans("Ending Balance") ." - ".$_POST['TransToDate'], "colspan=2");
...
if (($pbal = round2($pbal, user_price_dec())) != 0 && $_POST['Dimension'] == 0 && $_POST['Dimension2'] == 0)
    display_warning(trans("The Opening Balance is not in balance, probably due to a non closed Previous Fiscalyear."));
div_end();
