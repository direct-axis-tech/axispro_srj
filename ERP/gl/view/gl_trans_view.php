...
function display_gl_heading($myrow)
{
	...
	if ($journal)
	{
		...
		label_cell(sql2date($header["tran_date"]), "align='center'");
...
// Modified by Shajeer for displaying voided entries too
$void_entry = get_voided_entry($_GET['type_id'], $_GET['trans_no']);
$voided = ($void_entry!=null) ? true : false; 
  
$result = get_gl_trans($_GET['type_id'], $_GET['trans_no'],$voided);
...
if ($dim == 2)
	$th = array(_("Journal Date"), _("Account Code"), _("Account Name"), _("Cost Center")." 1", _("Dimension")." 2",
		_("Debit"), _("Credit"), _("Memo"));
elseif ($dim == 1)
	$th = array(_("Journal Date"), _("Account Code"), _("Account Name"), _("Cost Center"),
		_("Debit"), _("Credit"), _("Memo"));
else		
...
while ($myrow = db_fetch($result)) 
{
	...
	$counterpartyname = $myrow["person_name"];
	$counterparty_id = $counterpartyname ? ' : '.$myrow["person_id"] : '';
...
$param=$_GET['trans_no'].'-'.$_GET['type_id'];
?>
