...
page(trans($help_context = "Quick Entries"));
...
function submit_add_or_update_center2($add=true, $title=false, $async=false)
{
	echo "<center>";
	if ($add)
		submit('ADD_ITEM2', trans("Add new"), true, $title, $async);
	else {
		submit('UPDATE_ITEM2', trans("Update"), true, $title, $async);
		submit('RESET2', trans("Cancel"), true, $title, $async);
	}
	echo "</center>";
}
...
function can_process() 
{
	...
	{
		display_error( trans("The Quick Entry description cannot be empty."));
	...
	{
		display_error( trans("You can only use Balance Based together with Journal Entries."));
		set_focus('base_desc');
		return false;
	}
	if (!$bal_type && strlen($_POST['base_desc']) == 0) 
	{
		display_error( trans("The base amount description cannot be empty."));
		set_focus('base_desc');
		return false;
	}
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	{	
		...
		if ($selected_id != -1) 
		{
			...
			display_notification(trans('Selected quick entry has been updated'));
		} 
		else 
		{
			add_quick_entry($_POST['description'], $_POST['type'], 
				input_num('base_amount'), $_POST['base_desc'], get_post('bal_type', 0), $_POST['usage']);
			display_notification(trans('New quick entry has been added'));
		}
		$Mode = 'RESET';
	}
}
...
if ($Mode2=='ADD_ITEM2' || $Mode2=='UPDATE_ITEM2') 
{
	...
	elseif ($selected_id2 != -1) 
	{
		...
		display_notification(trans('Selected quick entry line has been updated'));
	} 
	else 
	{
		add_quick_entry_line($selected_id, $_POST['actn'], $_POST['dest_id'], input_num('amount', 0), 
			$_POST['dimension_id'], $_POST['dimension2_id'], get_post('memo'));
		display_notification(trans('New quick entry line has been added'));
	}
	$Mode2 = 'RESET2';
}
...
if ($Mode == 'Delete')
{
	if (!has_quick_entry_lines($selected_id))
	{
		...
		display_notification(trans('Selected quick entry has been deleted'));
		$Mode = 'RESET';
	}
	else
	{
		display_error( trans("The Quick Entry has Quick Entry Lines. Cannot be deleted."));
		set_focus('description');
	}
}
...
if ($Mode2 == 'BDel')
{
	...
	display_notification(trans('Selected quick entry line has been deleted'));
	$Mode2 = 'RESET2';
}
//-----------------------------------------------------------------------------------
if ($Mode == 'RESET')
{
	...
	$_POST['base_desc']= trans('Base Amount');
...
$th = array(trans("Description"), trans("Type"), trans("Usage"),  "", "");
table_header($th);
...
text_row_ex(trans("Description").':', 'description', 50, 60);
text_row_ex(trans("Usage").':', 'usage', 80, 120);

quick_entry_types_list_row(trans("Entry Type").':', 'type', null, true);

if (get_post('type') == QE_JOURNAL)
{
	yesno_list_row(trans("Balance Based"), 'bal_type', null, trans("Yes"), trans("No"), true);
}	
...
{
	yesno_list_row(trans("Period"), 'base_amount', null, trans("Monthly"), trans("Yearly"));
	gl_all_accounts_list_row(trans("Account"), 'base_desc', null, true);
}
else
{
	text_row_ex(trans("Base Amount Description").':', 'base_desc', 50, 60, '');
	amount_row(trans("Default Base Amount").':', 'base_amount', price_format(0));
}
end_table(1);
...
if ($selected_id != -1)
{
	display_heading(trans("Quick Entry Lines") . " - " . $_POST['description']);
	$result = get_quick_entry_lines($selected_id);

	start_table(TABLESTYLE2);
	$dim = get_company_pref('use_dimension');
	if ($dim == 2)
		$th = array(trans("Post"), trans("Account/Tax Type"), trans("Amount"), trans("Memo"), trans("Dimension"), trans("Dimension")." 2", "", "");
	elseif ($dim == 1)	
		$th = array(trans("Post"), trans("Account/Tax Type"), trans("Amount"), trans("Memo"), trans("Dimension"), "", "");
	else	
		$th = array(trans("Post"), trans("Account/Tax Type"), trans("Amount"), trans("Memo"), "", "");
	...
	while ($myrow = db_fetch($result)) 
	{
		...
		edit_button_cell("BEd".$myrow["id"], trans("Edit"));
		delete_button_cell("BDel".$myrow["id"], trans("Delete"));
		end_row();
	}
	end_table(1);
	...
	quick_actions_list_row(trans("Posted").":",'actn', null, true);
	...
	if ($actn == 't') 
	{
		//item_tax_types_list_row(trans("Item Tax Type").":",'dest_id', null);
		tax_types_list_row(trans("Tax Type").":", 'dest_id', null);
	} 
	else 
	{
		gl_all_accounts_list_row(trans("Account").":", 'dest_id', null, $_POST['type'] == QE_DEPOSIT || $_POST['type'] == QE_PAYMENT);
		if ($actn != '=') 
		{
			if ($actn == '%') 
				small_amount_row(trans("Part").":", 'amount', price_format(0), null, "%", user_exrate_dec());
			else
				amount_row(trans("Amount").":", 'amount', price_format(0));
		}
		text_row_ex(trans("Line memo").':', 'memo', 50, 256, '');
	}
	if ($dim >= 1) 
		dimensions_list_row(trans("Dimension").":", 'dimension_id', null, true, " ", false, 1);
	if ($dim > 1) 
