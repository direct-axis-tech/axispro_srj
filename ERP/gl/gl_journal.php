...
if (isset($_GET['ModifyGL'])) {
    $_SESSION['page_title'] = sprintf(trans("Modifying Journal Transaction # %d."),
        $_GET['trans_no']);
    $help_context = "Modifying Journal Entry";
} else
    $_SESSION['page_title'] = trans($help_context = "Journal Entry");
...
if (isset($_GET['AddedID'])) {
    ...
    display_notification_centered(trans("Journal entry has been entered") . " #$trans_no");

    hyperlink_newpage_params("../voucher_print/index.php","Print","voucher_id=$trans_no".'-0');

    display_note(get_gl_view_str($trans_type, $trans_no, trans("&View this Journal Entry")));

    reset_focus();
    hyperlink_params($_SERVER['PHP_SELF'], trans("Enter &New Journal Entry"), "NewJournal=Yes");
    ...
    display_notification_centered(trans("Journal entry has been updated") . " #$trans_no");
    hyperlink_newpage_params("../voucher_print/index.php","Print","voucher_id=$trans_no".'-0');
...
if (isset($_GET['NewJournal'])) {
    ...
    $is_cash_handover_journal = db_query("SELECT id FROM 0_cash_handover_requests WHERE trans_no = {$_GET['trans_no']}");
    $is_cash_handover_journal = $is_cash_handover_journal && $is_cash_handover_journal->fetch_assoc();

    if (!isset($_GET['trans_type']) || $_GET['trans_type'] != 0 || !empty($is_cash_handover_journal)) {
        display_error(trans("You can edit directly only journal entries created via Journal Entry page."));
        hyperlink_params("$path_to_root/gl/gl_journal.php", trans("Entry &New Journal Entry"), "NewJournal=Yes");
        display_footer_exit();
    }

    $assetsDepreciationGl =  check_empty_result("SELECT COUNT(*) FROM ".TB_PREF."stock_depreciation_details WHERE trans_type = ". db_escape($_GET['trans_type']) ." AND trans_no = ". db_escape($_GET['trans_no']) ."");
	if($assetsDepreciationGl) {
		$msg = sprintf(trans("#%s assets depreciation transactions are closed for further edition."), $trans_no);	
	   	display_error($msg, true);
		display_footer_exit();
	}

    $employeeDeductionGl =  check_empty_result("SELECT COUNT(*) FROM ".TB_PREF."emp_reward_deductions WHERE trans_no = ". db_escape($_GET['trans_no']) ."");
	if($employeeDeductionGl) {
		$msg = sprintf(trans("#%s Employees deduction transactions are closed for further edition."), $trans_no);	
	   	display_error($msg, true);
		display_footer_exit();
	}
...
function create_cart($type = 0, $trans_no = 0)
{
    ...
    if ($trans_no) {
        ...
        $cart->contract_id = $header['contract_id'];

        $result = get_gl_trans($type, $trans_no);

        if ($result) {
            while ($row = db_fetch($result)) {
                $_SESSION['journal_items']->line_transaction_id = $row['transaction_id'];
                ...
                if ($curr_amount) {
                    $cart->add_gl_item($row['account'], $row['dimension_id'], $row['dimension2_id'],
                        $curr_amount, $row['memo_'], '', $row['person_id'], null, $row['color_code']);
                }
                
                if (!empty($row['created_by'])) {
                    $cart->created_by = $row['created_by'];
                }

                if (!empty($row['transacted_at'])) {
                    $cart->transacted_at = $row['transacted_at'];
                }
            }
        }
        ...
        if (isset($tax_info['net_amount']))    // guess exempt sales/purchase if any tax has been found
        {
			...
			foreach ($cart->gl_items as $gl)
                if (!is_tax_account($gl->code_id) && !in_array(is_subledger_account($gl->code_id), [SLT_ACCOUNTS_REC, SLT_ACCOUNTS_PAY]))
                    $net_sum += $gl->amount;
...
if (isset($_POST['Process'])) {
    ...
    if (get_post('currency') != get_company_pref('curr_default'))
        if (isset($_POST['_ex_rate']) && !check_num('_ex_rate', 0.000001)) {
            display_error(trans("The exchange rate must be numeric and greater than zero."));
            set_focus('_ex_rate');
            $input_error = 1;
        }
    ...
    if(
        $_SESSION['journal_items']->has_taxes()
        && !check_value('taxable_trans')
        && empty($_POST['is_tax_confirmed'])
    ) {
        display_warning(trans("There are postings to one of the Tax account. Did you forgot to check 'Include in tax register' ?."));
        set_focus('taxable_trans');
        $input_error = 1;
        $_POST['is_tax_confirmed'] = 1;
        $Ajax->activate('is_tax_confirmed');
    }

    if (check_value('taxable_trans')) {
        if (!tab_visible('tabs', 'tax')) {
            display_warning(trans("Check tax register records before processing transaction or switch off 'Include in tax register' option."));
            ...
            if ($net_amount && !$_SESSION['journal_items']->has_taxes() && !$_SESSION['journal_items']->has_sub_accounts()) {
                display_error(trans("Cannot determine tax register to be used. You have to make at least one posting either to tax or customer/supplier account to use tax register."));
...
if (isset($_POST['Process'])) {
    ...
    foreach ($cart->gl_items as $gl_row) {

        if($gl_row->person_type_id==PT_CUSTOMER){
            runAutomaticAllocation($gl_row->person_id);
        }
    ...
    $journal = JournalTransaction::where('type', JournalTransaction::JOURNAL)
        ->where('trans_no', $trans_no)
        ->first();

    event(new JournalRecorded($journal));
...
function check_item_data()
{
    ...
    if (!get_post('code_id')) {
        display_error(trans("You must select GL account."));
        set_focus('code_id');
        return false;
    }
    if (is_subledger_account(get_post('code_id'))) {
        if (!get_post('person_id')) {
            display_error(trans("You must select subledger account."));
    ...
    if (!$_SESSION["wa_current_user"]->can_access('SA_BANKJOURNAL') && is_bank_account($_POST['code_id'])) {
        display_error(trans("You cannot make a journal entry for a bank account. Please use one of the banking functions for bank transactions."));
        set_focus('code_id');
        return false;
    }
...
function handle_update_item()
{
    if ($_POST['UpdateItem'] != "" && check_item_data()) {
        ...
        $_SESSION['journal_items']->line_transaction_id = get_post('LineTransactionID');
        ...
        $_SESSION['journal_items']->gl_items[$_POST['Index']]->line_transaction_id = get_post('LineTransactionID');
...
function handle_new_item()
{
    ...
    $_SESSION['journal_items']->add_gl_item($_POST['code_id'], $_POST['dimension_id'],
        $_POST['dimension2_id'], $amount, $_POST['LineMemo'], '', get_post('person_id'),null);
    unset($_SESSION['journal_items']->tax_info);
    line_start_focus();
}
...
tabbed_content_start('tabs', array(
    'gl' => array(trans('&GL postings'), true),
    'tax' => array(trans('&Tax register'), check_value('taxable_trans')),
));
...
switch (get_post('_tabs_sel')) {
    ...
    case 'tax':
        update_tax_info();
        br();
        display_heading(trans("Tax register record"));
        br();
        start_table(TABLESTYLE, "width=40%");
        date_row(trans("VAT date:"), 'tax_date', '', "colspan='3'");
        //tax_groups_list_row(trans("Tax group:"), 'tax_group');
        end_table(1);

        start_table(TABLESTYLE, "width=60%");
        table_header(array(trans('Name'), trans('Input Tax'), trans('Output Tax'), trans('Net amount')));
        $taxes = get_all_tax_types();
        while ($tax = db_fetch($taxes)) {
            start_row();
...
submit_center('Process', trans("Process Journal Entry"), true,
    trans('Process journal entry only if debits equal to credits'), 'default');
...
<style>
    ...
    table.tablestyle2 td:nth-child(3) {
        width: 15% !important;
    }
</style>
<?php
