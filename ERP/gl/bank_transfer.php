...
if (isset($_GET['ModifyTransfer'])) {
	...
	$_SESSION['page_title'] = trans($help_context = "Bank Account Transfer Entry");
}
...
if (list_updated('FromBankAccount') || isset($_POST['_DatePaid_changed'])) {
	$Ajax->activate('__bank_balance_row');
}
...
function gl_payment_controls($trans_no)
{
    ...
    start_table(TABLESTYLE2, "style='width:50% !important' id='gl_table' ", '5', '2', 'text-nowrap');
    table_section_title(trans("BANK TRANSFER"));
    ...
    bank_balance_row($_POST['FromBankAccount'], 'class="d-block text-center w-150px"', get_post('DatePaid'));
...
function check_valid_entries($trans_no)
{
	...
	if ($trans_no) {
		...
		if (
			$_POST['FromBankAccount'] != $old_from['bank_act']
			|| $_POST['ToBankAccount'] != $old_to['bank_act']
			|| $_POST['DatePaid'] != sql2date($old_from['trans_date'])
			|| abs(round2($old_from['amount'], 2)) != round2($amnt_tr, 2)
		) {
