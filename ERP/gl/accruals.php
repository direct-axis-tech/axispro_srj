...
$_SESSION['page_title'] = trans($help_context = "Revenue / Cost Accruals");
...
if (isset($_POST['go']) || isset($_POST['show']))
{
	...
	{
		display_error(trans("The amount can not be 0."));
		set_focus('amount');
		$input_error = 1;
	}
	elseif (input_num('periods', 0) < 1)
	{
		display_error(trans("The periods must be greater than 0."));
		set_focus('periods');
		$input_error = 1;
	}
	if ($input_error == 0)
	{
		...
		{
			display_error(trans("Some of the period dates are outside the fiscal year or are closed for further data entry. Create a new fiscal year first!"));
			set_focus('date_');
			$input_error = 1;
		}
		if ($input_error == 0)
		{
			...
			if (get_post('memo_') != "")
				...
				$memo = sprintf(trans("Accruals for %s"), $amount);
			if (isset($_POST['go']))
				begin_transaction();
			else
			{
				...
				$first_cols = array(trans("Date"), trans("Account"));
				if ($dim == 2)
				...
				$remaining_cols = array(trans("Debit"), trans("Credit"), trans("Memo"));
			...
			{
				commit_transaction();
				display_notification_centered(trans("Revenue / Cost Accruals have been processed."));
				$_POST['date_'] = $_POST['amount'] = $_POST['periods'] = "";
			}
			else
			{
				end_table(1);
				display_notification_centered(trans("Showing GL Transactions."));
			}
		}
	}
}
...
date_row(trans("Date"), 'date_', trans('First date of Accruals'), true, 0, 0, 0, null, true);
start_row();
label_cell(trans("Accrued Balance Account"), "class='label'");
gl_all_accounts_list_cells(null, 'acc_act', null, true, false, false, true);
end_row();
gl_all_accounts_list_row(trans("Revenue / Cost Account"), 'res_act', null, true);
...
amount_row(trans("Amount"), 'amount', null, null, viewer_link(trans("Search Amount"), $url, "", "", ICON_VIEW));

frequency_list_row(trans("Frequency"), 'freq', null);

text_row(trans("Periods"), 'periods', null, 3, 3);
textarea_row(trans("Memo"), 'memo_', null, 35, 3);

end_table(1);
submit_center_first('show', trans("Show GL Rows"));//,true,false,'process',ICON_SUBMIT);
submit_center_last('go', trans("Process Accruals"));//,true,false,'process',ICON_SUBMIT);
submit_js_confirm('go', trans("Are you sure you want to post accruals?"));
