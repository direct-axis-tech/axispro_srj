...
// save also in hidden field for testing during 'Reconcile'
	return is_closed_trans($row['type'], $row['trans_no']) ? "--" : checkbox(null, $name, $value, true, trans('Reconcile this transaction'))
...
function cheque_date($row) {
    return sql2date($row['chq_date']);
}

function cheque_no($row) {
    return $row['chq_no'];
}
...
start_row();
bank_accounts_list_cells(trans("Account:"), 'bank_account', null, true);

bank_reconciliation_list_cells(trans("Bank Statement:"), get_post('bank_account'),
	'bank_date', null, true, trans("New"));
end_row();
end_table();
