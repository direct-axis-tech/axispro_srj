...
page(trans($help_context = "Produce or Unassemble Finished Items From Work Order"), false, false, "", $js);
...
if (isset($_GET['AddedID']))
{
	...
	display_notification(trans("The manufacturing process has been entered."));
	
    display_note(get_trans_view_str($stype, $id, trans("View this Work Order")));
	...
	hyperlink_no_params("search_work_orders.php", trans("Select another &Work Order to Process"));
	br();
...
if ($wo_details === false)
{
	display_error(trans("The order number sent is not valid."));
	exit;
}
...
function can_process($wo_details)
{
	...
	{
		display_error(trans("The quantity entered is not a valid number or less then zero."));
		set_focus('quantity');
		return false;
	}
	...
	{
		display_error(trans("The production date cannot be before the release date of the work order."));
		set_focus('date_');
		return false;
	}
	...
	{
		display_error(trans("The production exceeds the quantity needed. Please change the Work Order."));
		set_focus('quantity');
		return false;
	}
	...
	if (($_POST['ProductionType'] == 0) && !$SysPrefs->allow_negative_stock())
	{
		...
		{
			display_error(trans("The unassembling cannot be processed because there is insufficient stock."));
			set_focus('quantity');
			return false;
		}
	}
	...
	if (($_POST['ProductionType'] == 1) && !$SysPrefs->allow_negative_stock())
	{
		...
		{
			if ($row['mb_flag'] == STOCK_TYPE_SERVICE) // service, non stock
				continue;

			if (check_negative_stock($row["stock_id"], -$row['units_req'] * input_num('quantity'), $row["loc_code"], $_POST['date_']))
			{
    			display_error( trans("The production cannot be processed because a required item would cause a negative inventory balance :") .
...
date_row(trans("Date:"), 'date_');
ref_row(trans("Reference:"), 'ref', '', $Refs->get_next(ST_MANURECEIVE, null, get_post('date_')), false, ST_MANURECEIVE);

if (!isset($_POST['ProductionType']))
	$_POST['ProductionType'] = 1;

yesno_list_row(trans("Type:"), 'ProductionType', $_POST['ProductionType'],
	trans("Produce Finished Items"), trans("Return Items to Work Order"));

small_qty_row(trans("Quantity:"), 'quantity', null, null, null, $dec);

textarea_row(trans("Memo:"), 'memo_', null, 40, 3);

end_table(1);

submit_center_first('Process', trans("Process"), '', 'default');
submit_center_last('ProcessAndClose', trans("Process And Close Order"), '', true);
