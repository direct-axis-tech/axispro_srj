...
page(trans($help_context = "Work Order Entry"), false, false, "", $js);


check_db_has_manufacturable_items(trans("There are no manufacturable items defined in the system."));
...
if (isset($_GET['AddedID']))
{
	...
	display_notification_centered(trans("The work order been added."));

    display_note(get_trans_view_str($stype, $id, trans("View this Work Order")), 0, 1);

	if ($_GET['type'] != WO_ADVANCED)
	{
		include_once($path_to_root . "/reporting/includes/reporting.inc");

		submenu_print(trans("&Print This Work Order"), ST_WORKORDER, $id, 'prtopt');
		submenu_print(trans("&Email This Work Order"), ST_WORKORDER, $id, null, 1);
    	display_note(get_gl_view_str($stype, $id, trans("View the GL Journal Entries for this Work Order")), 1);
    	$ar = array('PARAM_0' => $_GET['date'], 'PARAM_1' => $_GET['date'], 'PARAM_2' => $stype, 'PARAM_3' => '',
    		'PARAM_4' => (user_def_print_orientation() == 1 ? 1 : 0)); 
    	display_note(print_link(trans("Print the GL Journal Entries for this Work Order"), 702, $ar), 1);
		hyperlink_params("$path_to_root/admin/attachments.php", trans("Add an Attachment"), "filterType=$stype&trans_no=$id");
	}
...
if (isset($_GET['UpdatedID']))
{
	...
	display_notification_centered(trans("The work order been updated."));
	safe_exit();
}
...
if (isset($_GET['DeletedID']))
{
	...
	display_notification_centered(trans("Work order has been deleted."));
	safe_exit();
}
...
if (isset($_GET['ClosedID']))
{
	...
	display_notification_centered(trans("This work order has been closed. There can be no more issues against it.") . " #$id");
	safe_exit();
}
...
function safe_exit()
{
	...
	hyperlink_no_params("", trans("Enter a new work order"));
	hyperlink_no_params("search_work_orders.php", trans("Select an existing work order"));
...
function can_process()
{
	...
	{
		display_error( trans("The quantity entered is invalid or less than zero."));
		set_focus('quantity');
		return false;
	}
	...
	if (!($_POST['type'] == WO_ADVANCED))
	{
        ...
        {
        	display_error(trans("The selected item to manufacture does not have a bom."));
			set_focus('stock_id');
        	return false;
        }
    	...
    	{
    		display_error( trans("The labour cost entered is invalid or less than zero."));
			set_focus('Labour');
    		return false;
    	}
		if ($_POST['Costs'] == "")
    	...
    	{
    		display_error( trans("The cost entered is invalid or less than zero."));
			set_focus('Costs');
    		return false;
    	}
        ...
        if (!$SysPrefs->allow_negative_stock())
        {
        	if ($_POST['type'] == WO_ASSEMBLY)
        	{
            	...
            	while ($bom_item = db_fetch($result))
            	{
            		...
            		if (has_stock_holding($bom_item["ResourceType"]))
            		{
                		...
                		{
                			display_error(trans("The work order cannot be processed because there is an insufficient quantity for component:") .
                				" " . $bom_item["component"] . " - " .  $bom_item["description"] . ".  " . trans("Location:") . " " . $bom_item["location_name"]);
							set_focus('quantity');
        					return false;
                		}
            		}
        	...
        	elseif ($_POST['type'] == WO_UNASSEMBLY)
        	{
        		...
        		{
        			display_error(trans("The selected item cannot be unassembled because there is insufficient stock."));
					return false;
        		}
        	}
    	}
     ...
     else
     {
    	...
    	if (isset($selected_id))
    	{
    		...
    		{
				set_focus('quantity');
    			display_error(trans("The quantity cannot be changed to be less than the quantity already manufactured for this order."));
        		return false;
    		}
    	}
	}
...
if (isset($_POST['delete']))
{
	...
	{
		display_error(trans("This work order cannot be deleted because it has already been processed."));
		$cancel_delete = true;
	}
...
if (isset($selected_id))
{
	...
	if ($myrow === false)
	{
		echo trans("The order number sent is not valid.");
		safe_exit();
	}
	...
	{
		echo "<center>";
		display_error(trans("This work order is closed and cannot be edited."));
		safe_exit();
	}
	...
	label_row(trans("Reference:"), $_POST['wo_ref']);
	label_row(trans("Type:"), $wo_types_array[$_POST['type']]);
...
else
{
	...
	ref_row(trans("Reference:"), 'wo_ref', '', $Refs->get_next(ST_WORKORDER, null, get_post('date_')), false, ST_WORKORDER);

	wo_types_list_row(trans("Type:"), 'type', null);
}
...
if (get_post('released'))
{
	...
	label_row(trans("Item:"), $myrow["StockItemName"]);
	label_row(trans("Destination Location:"), $myrow["location_name"]);
}
else
{
	stock_manufactured_items_list_row(trans("Item:"), 'stock_id', null, false, true);
	if (list_updated('stock_id'))
		$Ajax->activate('quantity');

	locations_list_row(trans("Destination Location:"), 'StockLocation', null);
}
...
{
    qty_row(trans("Quantity Required:"), 'quantity', null, null, null, $dec);
    if ($_POST['released'])
    	label_row(trans("Quantity Manufactured:"), number_format($_POST['units_issued'], get_qty_dec($_POST['stock_id'])));
    date_row(trans("Date") . ":", 'date_', '', true);
	date_row(trans("Date Required By") . ":", 'RequDate', '', null, $SysPrefs->default_wo_required_by());
}
else
{
    qty_row(trans("Quantity:"), 'quantity', null, null, null, $dec);
    date_row(trans("Date") . ":", 'date_', '', true);
	hidden('RequDate', '');
	...
	gl_all_accounts_list_row(trans("Credit Labour Account"), 'cr_lab_acc', null);
	amount_row($wo_cost_types[WO_OVERHEAD], 'Costs');
	gl_all_accounts_list_row(trans("Credit Overhead Account"), 'cr_acc', null);

}

if (get_post('released'))
	label_row(trans("Released On:"),$_POST['released_date']);
...
if (isset($selected_id))
{
	...
	submit_cells('UPDATE_ITEM', trans("Update"), '', trans('Save changes to work order'), 'default');
	if (get_post('released'))
	{
		submit_cells('close', trans("Close This Work Order"),'','',true);
	}
	submit_cells('delete', trans("Delete This Work Order"),'','',true);

	echo "</tr></table>";
}
else
{
	submit_center('ADD_ITEM', trans("Add Workorder"), true, '', 'default');
}
