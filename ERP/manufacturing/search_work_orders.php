...
// curently outstanding simply means not closed
	$outstanding_only = 1;
	page(trans($help_context = "Search Outstanding Work Orders"), false, false, "", $js);
}
else
{
	$outstanding_only = 0;
	page(trans($help_context = "Search Work Orders"), false, false, "", $js);
}
//-----------------------------------------------------------------------------------
// Ajax updates
//
...
start_row();
ref_cells(trans("#:"), 'OrderId', '',null, '', true);
ref_cells(trans("Reference:"), 'OrderNumber', '',null, '', true);

locations_list_cells(trans("at Location:"), 'StockLocation', null, true);
...
stock_manufactured_items_list_cells(trans("for item:"), 'SelectedStockItem', null, true);
...
{
	return  $row['closed'] ? '<i>'.trans('Closed').'</i>' :
...
{
	return print_document_link($row['id'], trans("Print"), true, ST_WORKORDER, ICON_PRINT);
}
...
$cols = array(
	trans("#") => array('fun'=>'view_link', 'ord'=>''),
	trans("Reference"), // viewlink 2 ?
	trans("Type") => array('fun'=>'wo_type_name'),
	trans("Location"),
	trans("Item") => array('fun'=>'view_stock', 'ord'=>''),
	trans("Required") => array('fun'=>'dec_amount', 'align'=>'right'),
	trans("Manufactured") => array('fun'=>'dec_amount', 'align'=>'right'),
	trans("Date") => array('name'=>'date_', 'type'=>'date', 'ord'=>'desc'), 
	trans("Required By") => array('type'=>'date', 'ord'=>''),
...
$table->set_marker('check_overdue', trans("Marked orders are overdue."));
