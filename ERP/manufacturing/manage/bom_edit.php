...
page(trans($help_context = "Bill Of Materials"));

include_once($path_to_root . "/includes/date_functions.inc");
include_once($path_to_root . "/includes/ui.inc");
include_once($path_to_root . "/includes/data_checks.inc");

check_db_has_bom_stock_items(trans("There are no manufactured or kit items defined in the system."));

check_db_has_workcentres(trans("There are no work centres defined in the system. BOMs require at least one work centre be defined."));
...
function display_bom_items($selected_parent)
{
	...
	$th = array(trans("Code"), trans("Description"), trans("Location"),
		trans("Work Centre"), trans("Quantity"), trans("Units"),'','');
	table_header($th);
...
function on_submit($selected_parent, $selected_component=-1)
{
	...
	if ($selected_component != -1)
	{
		update_bom($selected_parent, $selected_component, $_POST['workcentre_added'], $_POST['loc_code'],
			input_num('quantity'));
		display_notification(trans('Selected component has been updated'));
		$Mode = 'RESET';
	}
	else
	{
		...
		if (!check_for_recursive_bom($selected_parent, $_POST['component']))
		{
				...
				$_POST['loc_code'], $selected_parent))
			{
				...
				display_notification(trans("A new component part has been added to the bill of material for this item."));
				$Mode = 'RESET';
			}
			else
			{
				/*The component must already be on the bom */
				display_error(trans("The selected component is already on this bom. You can modify it's quantity but it cannot appear more than once on the same bom."));
			}

		} //end of if its not a recursive bom
		else
		{
			display_error(trans("The selected component is a parent of the current item. Recursive BOMs are not allowed."));
		}
	}
}
...
start_row();
stock_manufactured_items_list_cells(trans("Select a manufacturable item:"), 'stock_id', null, false, true);
end_row();
...
start_form();
	...
	if ($selected_id != -1)
	{
 		if ($Mode == 'Edit') {
			...
			label_row(trans("Component:"), $myrow["component"] . " - " . $myrow["description"]);
		}
	...
	{
		start_row();
		label_cell(trans("Component:"), "class='label'");
...
//	hidden('stock_id', $selected_parent);
	...
	locations_list_row(trans("Location to Draw From:"), 'loc_code', null);
	workcenter_list_row(trans("Work Centre Added:"), 'workcentre_added', null);
