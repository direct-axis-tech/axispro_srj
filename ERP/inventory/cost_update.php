...
if (isset($_GET['FixedAsset'])) {
	$_SESSION['page_title'] = trans($help_context = "FA Revaluation");
	$_POST['fixed_asset'] = 1;
} else {
	$_SESSION['page_title'] = trans($help_context = "Inventory Item Cost Update");
}
...
if (get_post('fixed_asset') == 1)
	...
	check_db_has_costable_items(trans("There are no costable inventory items defined in the system (Purchased or manufactured items)."));
...
if (isset($_POST['UpdateData']))
{
	...
	{
		display_error( trans("The entered cost is not numeric."));
		set_focus('material_cost');
   	 	$should_update = false;
	}
	elseif ($old_cost == $new_cost)
	{
   	 	display_error( trans("The new cost is the same as the old cost. Cost was not updated."));
   	 	$should_update = false;
	}
   	...
   	if ($should_update)
   	{
        ...
        display_notification(trans("Cost has been updated."));

        if ($update_no > 0)
        {
    		display_notification(get_gl_view_str(ST_COSTUPDATE, $update_no, trans("View the GL Journal Entries for this Cost Update")));
        }
...
amount_row(trans("Unit cost"), "material_cost", null, "class='tableheader2'", null, $dec1);

if ($myrow && $myrow["mb_flag"]==STOCK_TYPE_MANUFACTURED)
{
	amount_row(trans("Standard Labour Cost Per Unit"), "labour_cost", null, "class='tableheader2'", null, $dec2);
	amount_row(trans("Standard Overhead Cost Per Unit"), "overhead_cost", null, "class='tableheader2'", null, $dec3);
}
else
{
...
refline_list_row(trans("Reference line:"), 'refline', ST_COSTUPDATE, null, false, get_post('fixed_asset'));
textarea_row(trans("Memo"), 'memo_', null, 40, 4);

end_table(1);
div_end();
submit_center('UpdateData', trans("Update"), true, false, 'default');
