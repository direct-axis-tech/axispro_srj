...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	{
		$input_error = 1;
		display_error( trans("The location code must be five characters or less long (including converted special chars)."));
		set_focus('loc_code');
	} 
	elseif (strlen($_POST['location_name']) == 0) 
	{
		$input_error = 1;
		display_error( trans("The location name must be entered."));
	...
	if ($input_error != 1) 
	{
    	if ($selected_id != -1) 
    	{
			...
			display_notification(trans('Selected location has been updated'));
    	} 
    	else 
    	{
    	...
    	/*selected_id is null cos no item selected on first time round so must be adding a	record must be submitting new entries in the new Location form */
    		...
    		add_item_location($_POST['loc_code'], $_POST['location_name'], $_POST['delivery_address'], 
				$_POST['phone'], $_POST['phone2'], $_POST['fax'], $_POST['email'], $_POST['contact'], check_value('fixed_asset'));
			display_notification(trans('New location has been added'));
    	}
...
function can_delete($selected_id)
{
	...
	{
		display_error(trans("Cannot delete this location because item movements have been created using this location."));
		return false;
	}

	if (key_in_foreign_table($selected_id, 'workorders', 'loc_code'))
	{
		display_error(trans("Cannot delete this location because it is used by some work orders records."));
		return false;
	}

	if (key_in_foreign_table($selected_id, 'cust_branch', 'default_location'))
	{
		display_error(trans("Cannot delete this location because it is used by some branch records as the default location to deliver from."));
		return false;
	}
	
	if (key_in_foreign_table($selected_id, 'bom', 'loc_code'))
	{
		display_error(trans("Cannot delete this location because it is used by some related records in other tables."));
		return false;
	}
...
if ($Mode == 'Delete')
{
	...
	if (can_delete($selected_id)) 
	{
		...
		display_notification(trans('Selected location has been deleted'));
...
$th = array(trans("Location Code"), trans("Location Name"), trans("Address"), trans("Phone"), trans("Secondary Phone"), "", "");
...
while ($myrow = db_fetch($result)) 
{
	...
	inactive_control_cell($myrow["loc_code"], $myrow["inactive"], 'locations', 'loc_code');
 	edit_button_cell("Edit".$myrow["loc_code"], trans("Edit"));
 	delete_button_cell("Delete".$myrow["loc_code"], trans("Delete"));
	end_row();
}
	//END WHILE LIST LOOP
...
if ($selected_id != -1) 
{
	...
	hidden("loc_code");
	label_row(trans("Location Code:"), $_POST['loc_code']);
} 
else 
{ //end of if $selected_id only do the else when a new record is being entered
	text_row(trans("Location Code:"), 'loc_code', null, 5, 5);
}

text_row_ex(trans("Location Name:"), 'location_name', 50, 50);
text_row_ex(trans("Contact for deliveries:"), 'contact', 30, 30);

textarea_row(trans("Address:"), 'delivery_address', null, 34, 5);

text_row_ex(trans("Telephone No:"), 'phone', 32, 30);
text_row_ex(trans("Secondary Phone Number:"), 'phone2', 32, 30);
text_row_ex(trans("Facsimile No:"), 'fax', 32, 30);
email_row_ex(trans("E-mail:"), 'email', 50);
