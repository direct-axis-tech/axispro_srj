...
page(trans($help_context = "Foreign Item Codes"), false, false, "", $js);

include_once($path_to_root . "/includes/date_functions.inc");
include_once($path_to_root . "/includes/ui.inc");
include_once($path_to_root . "/includes/data_checks.inc");

check_db_has_purchasable_items(trans("There are no inventory items defined in the system."));
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM')
{
   	...
   	{
      	$input_error = 1;
      	display_error( trans("The quantity entered was not positive number."));
		set_focus('quantity');
   	}
     	...
     	if ($Mode == 'ADD_ITEM') 
       	{
    		...
    		display_notification(trans("New item code has been added."));
       	} else
       	{
			update_item_code($selected_id, $_POST['item_code'], $_POST['stock_id'],
				$_POST['description'], $_POST['category_id'], $_POST['quantity'], 1); 

    	  	display_notification(trans("Item code has been updated."));
       	}
		$Mode = 'RESET';
	}
}
...
if ($Mode == 'Delete')
{
	...
	display_notification(trans("Item code has been sucessfully deleted."));
	$Mode = 'RESET';
}
...
div_start('code_table');
	...
	$th = array(trans("EAN/UPC Code"), trans("Quantity"), trans("Units"),
        ...
        while ($myrow = db_fetch($result))
        {
            ...
            label_cell($myrow["cat_name"]);
		 	...
		 	edit_button_cell("Delete".$myrow['id'], trans("Delete"));
            end_row();
...
text_row(trans("UPC/EAN code:"), 'item_code', null, 20, 20);
qty_row(trans("Quantity:"), 'quantity', null, '', $units, $dec);
text_row(trans("Description:"), 'description', null, 50, 200);
stock_categories_list_row(trans("Category:"), 'category_id', null);
