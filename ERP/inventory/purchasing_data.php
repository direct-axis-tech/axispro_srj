...
page(trans($help_context = "Supplier Purchasing Data"), false, false, "", $js);
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM')
{
   	...
   	{
      	$input_error = 1;
      	display_error( trans("There is no item selected."));
		set_focus('stock_id');
   	}
   	elseif (!check_num('price', 0))
   	{
      	$input_error = 1;
      	display_error( trans("The price entered was not numeric."));
	set_focus('price');
   	}
   	elseif (!check_num('conversion_factor'))
   	{
      	$input_error = 1;
      	display_error( trans("The conversion factor entered was not numeric. The conversion factor is the number by which the price must be divided by to get the unit price in our unit of measure."));
		set_focus('conversion_factor');
   	}
   	elseif ($Mode == 'ADD_ITEM' && get_item_purchasing_data($_POST['supplier_id'], $_POST['stock_id']))
   	{
      	$input_error = 1;
      	display_error( trans("The purchasing data for this supplier has already been added."));
     	...
     	if ($Mode == 'ADD_ITEM') 
       	{
    		...
    		display_notification(trans("This supplier purchasing data has been added."));
       	} 
       	else
       	{
       		update_item_purchasing_data($selected_id, $_POST['stock_id'], input_num('price',0),
       			$_POST['suppliers_uom'], input_num('conversion_factor'), $_POST['supplier_description']);
    	  	display_notification(trans("Supplier purchasing data has been updated."));
       	}
		$Mode = 'RESET';
	}
}
...
if ($Mode == 'Delete')
{
	...
	display_notification(trans("The purchasing data item has been sucessfully deleted."));
	$Mode = 'RESET';
}
...
if ($mb_flag == -1)
{
	display_error(trans("Entered item is not defined. Please re-enter."));
  	$Ajax->activate('price_table');
	set_focus('stock_id');
}
else
{
    ...
    {
    	display_note(trans("There is no purchasing data set up for the part selected"));
    }
    else
    {
        start_table(TABLESTYLE, "width='65%'");

		$th = array(trans("Supplier"), trans("Price"), trans("Currency"),
			trans("Supplier's Unit"), trans("Conversion Factor"), trans("Supplier's Description"), "", "");
        ...
        while ($myrow = db_fetch($result))
        {
            ...
            label_cell($myrow["supplier_description"]);
		 	edit_button_cell("Edit".$myrow['supplier_id'], trans("Edit"));
		 	delete_button_cell("Delete".$myrow['supplier_id'], trans("Delete"));
            end_row();
...
{
	hidden('supplier_id');
	label_row(trans("Supplier:"), $supp_name);
}
else
{
	supplier_list_row(trans("Supplier:"), 'supplier_id', null, false, true);
	$_POST['price'] = $_POST['suppliers_uom'] = $_POST['conversion_factor'] = $_POST['supplier_description'] = "";
}
amount_row(trans("Price:"), 'price', null,'', get_supplier_currency($selected_id), $dec2);
text_row(trans("Suppliers Unit of Measure:"), 'suppliers_uom', null, 50, 51);

if (!isset($_POST['conversion_factor']) || $_POST['conversion_factor'] == "")
{
   	$_POST['conversion_factor'] = maxprec_format(1);
}
amount_row(trans("Conversion Factor (to our UOM):"), 'conversion_factor', null, null, null, 'max');
text_row(trans("Supplier's Code or Description:"), 'supplier_description', null, 50, 50);
