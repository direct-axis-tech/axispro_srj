...
page(trans($help_context = "Install/Update Languages"), false, false, "", $js);
...
function display_languages()
{
	...
	$th = array(trans("Language"), trans("Name"), trans("Encoding"), trans("Right To Left"),
		trans("Installed"), trans("Available"), trans("Default"), "", "");
	...
	if (function_exists('gettext'))
	{
		if (check_value('DisplayAll'))
			 array_insert($th, 7, trans("Supported"));
		start_table();
		check_row(trans('Display also languages not supported by server locales'), 'DisplayAll', null, true);
		end_table();
	}
	...
	foreach ($langs as $pkg_name => $lng)
	{
		...
		label_cell($charset);
		label_cell($rtl ? trans("Yes") : trans("No"));
		...
		if (function_exists('gettext') && check_value('DisplayAll'))
			label_cell($support ? trans("Yes") :trans("No"));

		if (!$available && ($lang != 'C'))	// manually installed language
			button_cell('Edit'.$id, trans("Edit"), trans('Edit non standard language configuration'),
				ICON_EDIT);
		elseif (check_pkg_upgrade($installed, $available)) // outdated or not installed language in repo
			button_cell('Update'.$pkg_name, $installed ? trans("Update") : trans("Install"),
				trans('Upload and install latest language package'), ICON_DOWN);
		else
			label_cell('');
		...
		if (($id !== null) && ($lang != $currlang) && ($lang != 'C')) {
			...
			submit_js_confirm('Delete'.$id, 
				sprintf(trans("You are about to remove language \'%s\'.\nDo you want to continue ?"),
					$lang_name));
		} else
			label_cell('');
		end_row();
	...
	end_table();
	display_note(trans("The marked language is the current language which cannot be deleted."), 0, 0, "class='currentfg'");
	br();
	submit_center_first('Refresh', trans("Update default"), '', null);

	submit_center_last('Add', trans("Add new language manually"), '', false);
...
function check_data()
{
	...
	if (get_post('code') == '' || get_post('name') == '' || get_post('encoding') == '') {
		display_error(trans("Language name, code nor encoding cannot be empty"));
		return false;
	}
	$id = array_search_value($_POST['code'], $installed_languages, 'code');
	if ($id !== null && $installed_languages[$id]['package'] != null) {
		display_error(trans('Standard package for this language is already installed. If you want to install this language manually, uninstall standard language package first.'));
		return false;
	}
	return true;
}
...
function display_language_edit($selected_id)
{
	...
	text_row_ex(trans("Language Code"), 'code', 20);
	text_row_ex(trans("Language Name"), 'name', 20);
	text_row_ex(trans("Encoding"), 'encoding', 20);

	yesno_list_row(trans("Right To Left"), 'rtl', null, "", "", false);
	yesno_list_row(trans("Default Language"), 'dflt', null, "", "", false);

	file_row(trans("Language File") . " (PO)", 'uploadfile');
	file_row(trans("Language File") . " (MO)", 'uploadfile2');

	end_table(0);
	display_note(trans("Select your language files from your local harddisk."), 0, 1);
