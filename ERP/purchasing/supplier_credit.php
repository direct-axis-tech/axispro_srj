...
if (isset($_GET['New']))
{
	...
	if (isset($_GET['invoice_no']))
	{
		...
		$_SESSION['page_title'] = trans("Supplier Credit Note");
...
if (isset($_GET['AddedID'])) 
{
    ...
    echo "<center>";
    display_notification_centered(trans("Supplier credit note has been processed."));
    display_note(get_trans_view_str($trans_type, $invoice_no, trans("View this Credit Note")));

	display_note(get_gl_view_str($trans_type, $invoice_no, trans("View the GL Journal Entries for this Credit Note")), 1);

    hyperlink_params($_SERVER['PHP_SELF'], trans("Enter Another Credit Note"), "New=1");
...
function check_data()
{
	...
	{
		display_error(trans("The credit note cannot be processed because the there are no items or values on the invoice.  Credit notes are expected to have a charge."));
		set_focus('');
		return false;
	}
	...
	{
		display_error(trans("The credit note as entered cannot be processed because the date entered is not valid."));
		set_focus('tran_date');
		return false;
	} 
	...
	if (!$SysPrefs->allow_negative_stock()) {
		foreach ($_SESSION['supp_trans']->grn_items as $n => $item) {
			...
			{
				if (check_negative_stock($item->item_code, -$item->this_quantity_inv, null, $_SESSION['supp_trans']->tran_date, null, $item->maid_id))
				{
					$stock = get_item($item->item_code);
					display_error(trans("The return cannot be processed because there is an insufficient quantity for item:") .
						" " . $stock['stock_id'] . " - " . $stock['description'] . " - " .
						trans("Quantity On Hand") . " = " . number_format2(get_qoh_on_date($stock['stock_id'], null,
...
function handle_commit_credit_note()
{
	...
	process_cart($supp_trans, function (&$supp_trans) {
		if (isset($_POST['invoice_no']))
			$invoice_no = add_supp_invoice($supp_trans, $_POST['invoice_no']);
		else
			$invoice_no = add_supp_invoice($supp_trans);
...
function check_item_data($n)
{
	...
	{
		display_error(trans("The quantity to credit must be numeric and greater than zero."));
	...
	{
		display_error(trans("The price is either not numeric or negative."));
...
function commit_item_data($n)
{
	...
	$supp_trans->add_grn_to_trans(
		...
		input_num('This_QuantityCredited'.$n),
		$_POST['order_price'.$n],
...
br();
submit_center('PostCreditNote', trans("Enter Credit Note"), true, '', 'default');
br();
