...
page(trans($help_context = "View Purchase Order"), true, false, "", $js);


if (!isset($_GET['trans_no']))
{
	die ("<br>" . trans("This page must be called with a purchase order number to review."));
}

display_heading(trans("Purchase Order") . " #" . $_GET['trans_no']);
...
$th = array(trans("Item Code"), trans("Item Description"), trans("Quantity"), trans("Unit"), trans("Price"),
	trans("Requested By"), trans("Line Total"), trans("Quantity Received"), trans("Quantity Invoiced"));
table_header($th);
$total = $k = 0;
$overdue_items = false;
...
if ($overdue_items)
	display_note(trans("Marked items are overdue."), 0, 0, "class='overduefg'");
...
if (db_num_rows($grns_result) > 0)
{
    ...
    $th = array(trans("#"), trans("Reference"), trans("Delivered On"));
    table_header($th);
...
if (db_num_rows($invoice_result) > 0)
{
    ...
    start_table(TABLESTYLE);
    $th = array(trans("#"), trans("Date"), trans("Total"));
    table_header($th);
