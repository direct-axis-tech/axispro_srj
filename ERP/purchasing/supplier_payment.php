...
page(trans($help_context = "Supplier Payment Entry"), false, false, "", $js);
...
if (!isset($_POST['bank_account'])) { // first page call
	...
	if (isset($_GET['PInvoice'])) {
		...
		if ($inv) {
			...
			display_error(trans("Invalid purchase invoice number."));
	}
}
if (isset($_GET['AddedID'])) {
	$payment_id = $_GET['AddedID'];

   	display_notification_centered( trans("Payment has been sucessfully entered"));

	submenu_print(trans("&Print This Remittance"), ST_SUPPAYMENT, $payment_id."-".ST_SUPPAYMENT, 'prtopt');
	submenu_print(trans("&Email This Remittance"), ST_SUPPAYMENT, $payment_id."-".ST_SUPPAYMENT, null, 1);

	submenu_view(trans("View this Payment"), ST_SUPPAYMENT, $payment_id);
    display_note(get_gl_view_str(ST_SUPPAYMENT, $payment_id, trans("View the GL &Journal Entries for this Payment")), 0, 1);

	submenu_option(trans("Enter another supplier &payment"), "/purchasing/supplier_payment.php?supplier_id=".$_POST['supplier_id']);
	submenu_option(trans("Enter Other &Payment"), "/gl/gl_bank.php?NewPayment=Yes");
	submenu_option(trans("Enter &Customer Payment"), "/sales/customer_payments.php");
...
function check_inputs()
{
	...
	{
		display_error(trans("The entered discount is invalid or less than zero."));
		set_focus('amount');
		return false;
	}
	...
	{
		display_error(trans("The total of the amount and the discount is zero or negative. Please enter positive values."));
		set_focus('amount');
		return false;
	}
	...
	{
		display_error(trans("The entered bank amount is zero or negative."));
	...
	{
		display_error(sprintf(trans("The total bank amount exceeds allowed limit (%s)."), price_format($limit)));
		set_focus('amount');
		return false;
	}
...
start_form();
    ...
    supplier_list_row(trans("Payment To:"), 'supplier_id', null, false, true);
    ...
    bank_accounts_list_row(trans("From Bank Account:"), 'bank_account', null, true);

	bank_balance_row($_POST['bank_account']);

	table_section(2);

    date_row(trans("Date Paid") . ":", 'DatePaid', '', true, 0, 0, 0, null, true);

    ref_row(trans("Reference:"), 'ref', '', $Refs->get_next(ST_SUPPAYMENT, null,
	...
	{
		amount_row(trans("Bank Amount:"), 'bank_amount', null, '', $bank_currency);
	}

	amount_row(trans("Bank Charge:"), 'charge', null, '', $bank_currency);
	...
	amount_row(trans("Amount of Discount:"), 'discount', null, '', $supplier_currency);
	amount_row(trans("Amount of Payment:"), 'amount', null, '', $supplier_currency);
	textarea_row(trans("Memo:"), 'memo_', null, 55, 4);
	end_table(1);

	submit_center('ProcessSuppPayment',trans("Enter Payment"), true, '', 'default');
