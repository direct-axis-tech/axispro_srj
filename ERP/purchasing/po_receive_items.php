...
page(trans($help_context = "Receive Purchase Order Items"), false, false, "", $js);
...
if (isset($_GET['AddedID']))
{
	...
	display_notification_centered(trans("Purchase Order Delivery has been processed"));

	display_note(get_trans_view_str($trans_type, $grn, trans("&View this Delivery")));
	
    $clearing_act = get_company_pref('grn_clearing_act');
	if ($clearing_act)	
		display_note(get_gl_view_str($trans_type, $grn, trans("View the GL Journal Entries for this Delivery")), 1);

	hyperlink_params("$path_to_root/purchasing/supplier_invoice.php", trans("Entry purchase &invoice for this receival"), "New=1");

	hyperlink_no_params("$path_to_root/purchasing/inquiry/po_search.php", trans("Select a different &purchase order for receiving items against"));
...
{
	die (trans("This page can only be opened if a purchase order has been selected. Please select a purchase order first."));
}
...
function display_po_receive_items()
{
	...
	$dimension = Dimension::find($po->dimension) ?: Dimension::make();

	div_start('grn_items');
    start_table(TABLESTYLE, "colspan=7 width='90%'");

	$is_maid_select_enabled = is_maid_select_enabled($po->dimension, $po->line_items);
	...
	$th[] = trans("Units");
	$th[] = trans("Received");
	$th[] = trans("Outstanding");
	$th[] = trans("This Delivery");
	$th[] = trans("Price");
	$th[] = trans("Total");
    ...
    {
       	foreach ($po->line_items as $ln_itm)
       	{
			...
			if ($is_so_line_ref_enabled) {
				label_cell($ln_itm->so_line_reference ?: '--');
			}
			...
			if ($is_maid_select_enabled)
				label_cell(Labour::whereId($ln_itm->maid_id)->value('name') ?: 'NA');
	...
	$taxes = $po->get_taxes(input_num('freight_cost'), true);
	
	$tax_total = display_edit_tax_items($taxes, $colspan, $po->tax_included);
...
function can_process()
{
	...
	{
        display_error(trans("There is nothing to process. Please enter valid quantities greater than zero."));
    	return false;
	}
    ...
    {
    	display_error(trans("Entered quantities cannot be greater than the quantity entered on the purchase order including the allowed over-receive percentage") . " (" . $SysPrefs->over_receive_allowance() ."%)."
    		. "<br>" .
    	 	trans("Modify the ordered items on the purchase order if you wish to increase the quantities."));
    	return false;
    }
...
function process_receive_po()
{
	...
	if (check_po_changed())
	{
		display_error(trans("This order has been changed or invoiced since this delivery was started to be actioned. Processing halted. To enter a delivery against this purchase order, it must be re-selected and re-read again to update the changes made by the other user."));

		hyperlink_no_params("$path_to_root/purchasing/inquiry/po_search.php",
		 trans("Select a different purchase order for receiving goods against"));

		hyperlink_params("$path_to_root/purchasing/po_receive_items.php", 
			 trans("Re-Read the updated purchase order for receiving goods against"),
...
display_heading(trans("Items to Receive"));
display_po_receive_items();

echo '<br>';
submit_center_first('Update', trans("Update"), '', true);
submit_center_last('ProcessGoodsReceived', trans("Process Receive Items"), trans("Clear all GL entry fields"), 'default');
