...
page(trans($help_context = "Supplier Inquiry"), isset($_GET['supplier_id']), false, "", $js);
...
function prt_link($row)
{
  	if ($row['type'] == ST_SUPPAYMENT || $row['type'] == ST_BANKPAYMENT || $row['type'] == ST_SUPPCREDIT) 
 		return print_document_link($row['trans_no']."-".$row['type'], trans("Print Remittance"), true, ST_SUPPAYMENT, ICON_PRINT);
}
...
if (!$page_nested)
	supplier_list_cells(trans("Select a supplier:"), 'supplier_id', null, true, true, false, true);
...
$cols = array(
			...
			trans("Ref #") => array('fun'=>'trans_view', 'ord'=>'', 'align'=>'right'),
			trans("Reference")=> 'skip',
			trans("Supplier"),
